...
function BattleDramaModel:__init(ctrl)
    ...
    self.buff_data = {}
    self.last_dun_id = 0
    self.auto_combat = nil --是否勾选自动挑战  1  0

    self.is_auto_combat = nil --是否勾选自动挑战  1  0
...
function BattleDramaModel:setIsBattleAutoFight(status)
    if status then
        self.is_auto_combat = 1
        BattleDramaController:getInstance():send13003(1)
    else
        self.is_auto_combat = 0
        BattleDramaController:getInstance():send13003(2)
    end

end

function BattleDramaModel:getIsBattleAutoFight()
    if self.is_auto_combat == 1 then
        return 1
    else
        return 0
    end
...
function BattleDramaModel:setQuickData(data)
    ...
    if self.drama_data then
        ...
        self.is_auto_combat = data.is_auto_combat
    end
    self:checkRedPoint()
