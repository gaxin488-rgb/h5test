...
function BattlDramaHookRewardWindow:open_callback()
    ...
    main_container:getChildByName("get_title"):setString(TI18N("语言_c_121"))
    ...
    self.vip_notice_label:setString(TI18N("语言_c_2464"))
    ...
    self.time_xiegang = main_container:getChildByName("time_xiegang")
    self.time_value2 = main_container:getChildByName("time_value2")
...
function BattlDramaHookRewardWindow:analysisList()
    local list = {}
    for i,v in ipairs(self.data.items) do
        local item = clone(v)
        local inner = self.scroll_view.scroll_view:getInnerContainer()
        local pos = inner:convertToWorldSpace(cc.p(0, 0))
        local x = pos.x + 120*(i%4 == 0 and 4 or i%4) - 60
        local y = pos.y + inner:getContentSize().height - math.floor(i/4) * 145 - 72
        item.w_pos =  cc.p(x, y)
        list[#list + 1] = item
    end
    return list
...
function BattlDramaHookRewardWindow:closeThisWindow()
    ...
    playButtonSound2()
    GlobalEvent:getInstance():Fire(MainuiEvent.GET_ITEM_EXECUTE_FLY_EVENT, self:analysisList())
...
function BattlDramaHookRewardWindow:openRootWnd(data)
    ...
    if data then
        ...
        if self.result_type == 1 then
            self.time_type_title:setString(TI18N("语言_c_2465"))
        elseif self.result_type == 2 then
            self.time_type_title:setString(TI18N("语言_c_2466"))
        end
        self.data = data
        ...
        -- 暂时屏蔽vip字样显示
        -- self.vip_notice_label:setString(TI18N("语言_c_2467"))
        if data.vip_buff then
            if data.vip_buff == 0 then
                self.vip_notice_label:setString(TI18N("语言_c_2468"))
            else
                if data.honor_buff and data.honor_buff ~= 0 then
                    self.vip_notice_label:setString(TI18N("语言_c_2464"))
                else
                    self.vip_notice_label:setString(TI18N("语言_c_2469"))
                end
            end
        end
    end
...
function BattlDramaHookRewardWindow:setBaseInfo(data)
    ...
    self.level:setString(string.format(TI18N("语言_c_4712"),data.new_lev))
    local time1 = Config.DungeonData.data_drama_const.profit_time_max.val
    local role_vo = RoleController:getInstance():getRoleVo()
    local vipLevel = role_vo.vip_lev
    local time2 = 0
    if vipLevel and Config.VipData.data_get_vip_info[vipLevel] and Config.VipData.data_get_vip_info[vipLevel].val then
        time2 = Config.VipData.data_get_vip_info[vipLevel].val[3][2] * 3600
    end
    local TotalDuration = time1 + time2
    -- self.time_value:setString(TimeTool.GetTimeFormat(data.time)/TimeTool.GetTimeFormat(TotalDuration))
    self.time_value:setString(TimeTool.GetTimeFormat(data.time))
    self.time_xiegang:setString("/")
    self.time_value2:setString(TimeTool.GetTimeFormat(TotalDuration))
...
function BattlDramaHookRewardItem:setData(data)
    if data then
        ...
        -- end
            self.item_name_label:setVisible(false)
    end
end
