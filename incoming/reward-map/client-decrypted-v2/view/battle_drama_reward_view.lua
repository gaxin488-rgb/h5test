...
function BattleDramaRewardWindow:__init(dun_id)
    ...
    self.res_list = {
        {path = PathTool.getPlistImgForDownLoad("bigbg", "txt_cn_bigbg_61"), type = ResourcesType.single },
...
function BattleDramaRewardWindow:open_callback()
    ...
    container:getChildByName("win_title"):setString(TI18N("语言_c_2475"))

    self.panel_bg = self.container:getChildByName("panel_bg")
    
    if not self.item_load then
        local res =  PathTool.getTargetRes("bigbg","txt_cn_bigbg_61", false, false)
...
function BattlDramaRewardItem:setData(data)
    ...
    local str = string.format(TI18N("语言_c_4761"),drama_dungeon_info.floor,data.cur_dun,drama_dungeon_info.floor)
    ...
    if cur_dun >= drama_dungeon_info.floor then
        if is_received then
            ...
            self.comfirm_btn_label:setString(TI18N("语言_c_109"))
            self.comfirm_btn_label:setTextColor(cc.c4b(0xf8,0xef,0x76,255))
    	    self.comfirm_btn_label:enableOutline(cc.c4b(0x5c,0x27,0x05,0xff), 2)
        end
    else 
        --前往
        self.img_received:setVisible(false)
        ...
        self.comfirm_btn_label:setString(TI18N("语言_c_108"))
        self.comfirm_btn_label:setTextColor(cc.c4b(0xf9,0xee,0x70,255))
    	self.comfirm_btn_label:enableOutline(cc.c4b(0x5c,0x27,0x05,0xff), 2)
    end
