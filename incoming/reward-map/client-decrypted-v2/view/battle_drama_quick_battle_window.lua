...
function BattlDramaQuickBattleWindow:open_callback()
    ...
    self.title_label:setString(TI18N("语言_c_4867"))
    self.holid_up:getChildByName("Text_1"):setString(TI18N("语言_c_2486"))
    ...
    self.quick_btn = createButton(self.main_container, TI18N("语言_c_2479"), self.main_container:getContentSize().width/2,426, cc.size(220, 64), PathTool.getResFrame("common", "common_1017"), 26, cc.c4b(248,239,118,0xff))
    self.quick_btn:enableOutline(cc.c4b(92,39,5,0xff), 2)

    self.source_btn = self.main_container:getChildByName("source_btn")
    self.source_btn:setVisible(false)
    --self.source_btn:getChildByName("label"):setString(TI18N("语言_c_2487"))

    self.main_container:getChildByName("use_title"):setString(TI18N("语言_c_2488"))
    ...
    self.desc = createRichLabel(22, cc.c4b(0x68,0x45,0x2A,255), cc.p(0, 1), cc.p(0, 0), -4, nil, self.scroll_size.width - 4) 
    self.scroll_view:addChild(self.desc)

    self.btn_buy_quick = self.main_container:getChildByName("btn_buy_quick")
    self.btn_buy_quick:getChildByName("Text_2"):setString(TI18N("语言_c_1858"))
...
function BattlDramaQuickBattleWindow:updateData()
    ...
    if quickdata then
        ...
        local tips_str = string.format(TI18N("语言_c_2489"), hook_max_time)
        tips_str = tips_str .. "\n" .. string.format(TI18N("语言_c_2490"))
        if status then
            tips_str = tips_str .. string.format(TI18N("语言_c_2491"))
            self.btn_buy_quick:setVisible(false)
        else
            tips_str = tips_str .. string.format(TI18N("语言_c_2492"))
            self.btn_buy_quick:setVisible(true)
        end
        self.desc:setString(tips_str)
            ...
            if event_type == ccui.TouchEventType.ended then
                ...
                elseif self.quick_battle_status == 3 then
                    if not self.privilege_status then -- 未开通特权，则弹出提示
                        ...
                        --MallController:getInstance():openChargeShopWindow(true, MallConst.Charge_Shop_Type.Privilege)
                        controller:openDramBattleQuickView(false)
                        --[[local str = TI18N("语言_c_2493")
                        local function fun()
                            VipController:getInstance():openVipMainWindow(true, VIPTABCONST.PRIVILEGE)
                            controller:openDramBattleQuickView(false)
                        end
                        CommonAlert.show(str,TI18N("语言_c_645"), fun,TI18N("语言_c_62"),nil, CommonAlert.type.rich,nil, nil, nil, true)--]]
                    else
                        self:send13004()
                    end
                else
                    self.cost = self.cost or 0
                    local str = TI18N("语言_c_2494")..string.format("<img src=%s visible=true scale=0.3 /><div>x%s</div>", PathTool.getItemRes(Config.ItemData.data_assets_label2id.gold), self.cost)
                    local function fun()
                        self:send13004()
                    end
                    CommonAlert.show(str,TI18N("语言_c_645"), fun,TI18N("语言_c_62"),nil, CommonAlert.type.rich,nil, nil, nil, true)
...
function BattlDramaQuickBattleWindow:send13004()
    ...
    if (cur_energy + qingbao_val) > max_energy then
        ...
        local str = string.format(TI18N("语言_hol_ge_03_1"), cur_energy, max_energy)
...
function BattlDramaQuickBattleWindow:checkQuickBattle()
    if self.quick_data then
        ...
        local str = string.format("%s<div fontColor=#249003>%s</div>",TI18N("语言_c_2495"),combat_num)

        if self.privilege_status then
            str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("语言_c_2495"),combat_num, privilege_num)
        end
        ...
        if self.quick_data.is_holiday == 1 or self.privilege_status == true then
            if self.quick_data.is_holiday == 1 and self.privilege_status == true then --活动与特权同时存在
                ...
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("语言_c_2495"),combat_num, holid_num+privilege_num)
            elseif self.privilege_status == true then --仅有特权
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("语言_c_2495"),combat_num, privilege_num)
            else
                local holid_num = Config.DungeonData.data_drama_const.hd_fast_combat_buy_time.val - self.quick_data.fast_combat_w_num
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("语言_c_2495"),combat_num, holid_num)
            end
        end
        ...
        if self.quick_data.fast_combat_free_num > 0 then --代表是免费的
            ...
            if self.privilege_status then -- 特权开启
                ...
                if Config.DungeonData.data_drama_quick_cost[index] then -- 普通次数未用完
                    ...
                    btn_str = string.format(TI18N("语言_c_2497"),PathTool.getItemRes(Config.ItemData.data_assets_label2id.gold), self.cost)
                ...
                if Config.DungeonData.data_drama_quick_cost[index] then
                    ...
                    if self.quick_data.is_holiday == 1 and self.quick_data.fast_combat_w_num < const.hd_fast_combat_buy_time.val then 
                        ...
                        btn_str = string.format(TI18N("语言_c_2497"),PathTool.getItemRes(const.hd_fast_combat_lose.val[1][1]), self.cost)
                    else
                        self.quick_battle_status = 3
                        btn_str = TI18N("语言_c_2498")
                    end
                end
            end 
        end
