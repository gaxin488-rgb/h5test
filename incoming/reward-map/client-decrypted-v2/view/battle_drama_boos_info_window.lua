...
function BattlDramaBossInfoWindow:open_callback()
    ...
    self.boss_name_desc:setString(TI18N("语言_c_4481"))
    ...
    self.diffcult_label_desc:setString(TI18N("语言_c_2053"))
    ...
    self.title_label:setString(TI18N("语言_c_2512"))
    ...
    self.free_swap_label:setString(TI18N("语言_c_2513"))
...
function BattlDramaBossInfoWindow:updateData()
    if self.dungeon_data then
        ...
        local str = string.format(TI18N("语言_c_2514"),cur_dungeon_data.auto_num, Config.DungeonData.data_drama_const["swap_time"].val)
        if cur_dungeon_data.auto_num >= Config.DungeonData.data_drama_const["swap_time"].val then
            str = string.format(TI18N("语言_c_2515"),cur_dungeon_data.auto_num, Config.DungeonData.data_drama_const["swap_time"].val)
        end
        ...
        -- local str = string.format(TI18N("语言_c_2516"),PathTool.getItemRes(item_icon),num)
...
function BattlDramaBossInfoWindow:updateBtnStatus(status)
    if status == 3 then --已通关
        ...
        -- self.ack_button:setTitleText(TI18N("语言_c_2517"))
        ...
        --self.swap_num_label:setVisible(false)
        -- setChildUnEnabled(true, self.ack_button)
        -- self.ack_button:setTitleText(TI18N("语言_c_457"))
...
function BattlDramaBossInfoWindow:register_event()
    ...
    if not self.update_dram_boss_data_event then
        self.update_dram_boss_data_event = GlobalEvent:getInstance():Bind(Battle_dramaEvent.BattleDrama_Boss_Update_Data, function(data)
            if data then
                local str = string.format(TI18N("语言_c_4483"),data.auto_num, Config.DungeonData.data_drama_const["swap_time"].val)
                if data.auto_num >= Config.DungeonData.data_drama_const["swap_time"].val then
                    str = string.format(TI18N("语言_c_2505"),data.auto_num, Config.DungeonData.data_drama_const["swap_time"].val)
                end
                self.swap_num_label:setString(str)
                local num = BackpackController:getInstance():getModel():getBackPackItemNumByBid(Config.DungeonData.data_drama_const["swap_item"].val)
                -- str = string.format(TI18N("语言_c_4484"), num)
                -- self.swap_label:setString(str)
...
function BattlDramaBossInfoWindow:checkSwapAlert()
    ...
    if not self.is_free then
        if num <= 0 then --扫荡券不足
            ...
            local str = string.format(TI18N("语言_c_2504"),PathTool.getItemRes(loss[1][1]),loss[1][2])
