...
function BttleTopDramaView:ctor(battle_res_id,battle_type)
    ...
    self.btn_count = 0
    self.fly_item_sum = 0
    self.left_or_right = true --true表示从左到右 false表示从右到左
    self:initConfig()
    self:createRootWnd()
    self:loadResources()
end
...
function BttleTopDramaView:readyCreateFunIcon()
    ...
    local delay_fun_list = {"updateWorldMapInfo","updatePassCahpterInfo", "updateTaskInfo", "updateQingbaoInfo", "updateDramaHallows", "updateOnlineGiftInfo", "updateWorldLevelIcon"}
...
function BttleTopDramaView:receiveChangeData()
    ...
    if not self.online_show_item then
        if self.online_gift_node then
            ...
            self.online_show_item:setPosition(cc.p(26, 50))
        end
    end
    if self.online_show_item then
        ...
        if online_time >= _get_time_items[num].time then
            ...
            self.online_gift_node.label:setString(TI18N("语言_c_988"))
            end
        else
    ...
    if self.map_name_container and not _tolua_isnull(self.map_name_container) then
        ...
        if cur_data and cur_data.per_hook_items then
            ...
            for i, v in ipairs(per_hook_items) do
                ...
                num_str = num_str .. _string_format(TI18N("语言_c_29"), v[2])
            end
        end
    ...
    if _b_controller:getCtrlBattleScene() and self.drama_data then
        ...
        if _config and _config[self.drama_data.chapter_id] then
            ...
            if _b_controller:getCtrlBattleScene() then
                ...
                local config = Config.BattleBgData.data_info2[self.battle_type][map_id]
                if config and config.is_stratify == 1 then
                    map_bg_top_res = PathTool.getBattleSceneRes(_string_format("%s/map_bg2", map_id), false)
                    table.insert(self.res_list, { path = map_bg_top_res, type = ResourcesType.single })
                end
                self.map_resources_load:addAllList(self.res_list, function()
                    if _b_controller:getCtrlBattleScene() then
                        _b_controller:getCtrlBattleScene():updateBg(map_bg_res, m_bg_res, nil, map_bg_top_res)
                    end
...
function BttleTopDramaView:registerEvent()
    ...
    -- 对掉落详情物品列表进行监听
    -- self.drama_item_scrollview:addEventListener(function(sender, eventType)
    ...
    --         print("scrolling")
    --         print(self.drama_item_scrollview:isScrolling())
    --     elseif eventType == ccui.ScrollviewEventType.containerMoved then
    --         print("containerMoved")
    --     elseif eventType == ccui.ScrollviewEventType.autoscrollEnded then
    --         print("autoscrollEnded")
    --     end
    -- end)
end

function BttleTopDramaView:scrollerHandler()
...
function BttleTopDramaView:createSpecialIcon(res, content, load_type)
    ...
    content = content or ""
    local label = createLabel(20, cc.c4b(0xff,0xff,0xff,0xff), cc.c4b(0x00,0x00,0x00,0xff), 38, 2, content, container, 2, cc.p(0.5,0.5))
...
function BttleTopDramaView:createReceiveIcon(content)
    ...
    local bg = createSprite(PathTool.getResFrame("battledrama","battledrama_1014"),15,38,container,cc.p(0.5,0.5),LOADTEXT_TYPE_PLIST)
    content = content or ""
    local label = createLabel(24, cc.c4b(0x00,0xff,0x00,0xff), 2, 26, 3, content, container, 2, cc.p(0.5,0.5))
...
function BttleTopDramaView:updateWorldMapInfo()
    ...
    if self.world_map_node == nil then
        -- self.world_map_node = self:createSpecialIcon(PathTool.getResFrame("battledrama", "battledrama_1017"), TI18N("语言_c_2472"), LOADTEXT_TYPE_PLIST)
        ...
        -- local map_name_bg = createSprite(PathTool.getResFrame("battledrama","battledrama_map"),43,node_size.height/2,self.world_map_node,cc.p(0.5,0.5),LOADTEXT_TYPE_PLIST, 0)
        local map_name_bg = createScale9Sprite(PathTool.getResFrame("battledrama", "battledrama_map"), 143, node_size.height/2, LOADTEXT_TYPE_PLIST, self.world_map_node)
        map_name_bg:setAnchorPoint(cc.p(1, 0.5))
        map_name_bg:setCapInsets(cc.rect(89, 0, 5, 0))
        map_name_bg.size = map_name_bg.size or map_name_bg:getContentSize()
        -- map_name_bg:setContentSize(cc.size(self.map_left_bg:getContentSize().width, self.map_name_container:getContentSize().height))
        ...
        local name_label = createLabel(18,cc.c3b(255,255,255),cc.c3b(255,255,255),node_size.width/2 - 60,68,"",self.world_map_node,0,cc.p(0.5, 1))
        name_label.s_width = 98
        name_label.s_pos = cc.p(node_size.width/2 - 60, 68)
--         -- 小地图裁剪
        ...
        self.world_map_node.map_name_bg = map_name_bg

        local chapter_label = createLabel(18,cc.c3b(255,240,156),cc.c3b(255,240,156),node_size.width/2 - 60,92,"",self.world_map_node,0,cc.p(0.5, 1))
        self.world_map_node.chapter_label = chapter_label
        chapter_label.s_pos = cc.p(node_size.width/2 - 60, 92)
    ...
    if self.world_map_node then
        ...
        if self.drama_data then
            ...
            if self.world_map_node.name_label then
                ...
                if self.world_map_node.name_label:getContentSize().width > self.world_map_node.name_label.s_width then
                    local size = clone(self.world_map_node.map_name_bg.size)
                    size.width = size.width + (self.world_map_node.name_label:getContentSize().width - self.world_map_node.name_label.s_width)
                    self.world_map_node.map_name_bg:setContentSize(size)

                    local pos = clone(self.world_map_node.name_label.s_pos)
                    pos.x = pos.x - (self.world_map_node.name_label:getContentSize().width - self.world_map_node.name_label.s_width)/2
                    self.world_map_node.name_label:setPosition(cc.p(pos))

                    if self.world_map_node.chapter_label then
                        local pos = clone(self.world_map_node.chapter_label.s_pos) 
                        pos.x = pos.x - (self.world_map_node.name_label:getContentSize().width - self.world_map_node.name_label.s_width)/2
                        self.world_map_node.chapter_label:setPosition(cc.p(pos))
                    end
                end
            end

            if self.world_map_node.chapter_label then
                self.world_map_node.chapter_label:setString(string.format(TI18N("语言_c_2875"), drama_config.chapter_id))
            end
...
function BttleTopDramaView:updateTaskInfo(status)
    ...
    if self.task_icon_vo == nil then
        ...
        self.task_icon = self:createSpecialIcon(PathTool.getResFrame("battledrama", "battledrama_1016"), TI18N("语言_c_2473"), LOADTEXT_TYPE_PLIST)
...
function BttleTopDramaView:updateQingbaoInfo()
    ...
    if self.qingbao_node == nil then
        ...
        self.qingbao_container:getChildByName("label"):setString(TI18N("语言_c_2474"))        
        ...
        if self.battle_button_list then
            local pos1 = self.battle_button_list:getChildByName("image_bg"):convertToWorldSpace(cc.p(0, 0))
            if self.qingbao_node then
                -- body
                local node_pos2 = self.top_info_container:convertToNodeSpace(pos1)
                self.qingbao_node:setPositionY(node_pos2.y + 94 + 160)
                self.qingbao_node:setPositionX(node_pos2.x + 50)
            end
        end
    end
...
function BttleTopDramaView:updateRightBtnPos(is_right)
    if _tolua_isnull(self.top_info_container) then return end
    if _tolua_isnull(self.world_map_node) then return end
    local btn_dict = {}
    ...
    for i,v in ipairs(btn_list) do
        ...
        if is_right then
            ...
            local offset = (self.world_map_node.name_label:getContentSize().width - self.world_map_node.name_label.s_width)
            _x = top_size.width - 200 -(i - 1) * 94 - 40 - offset
        ...
        -- if v.node == self.qingbao_node then --把远航特权的按钮进行特殊处理
        --     v.node:setPosition(670, 0)
        -- end
    end
...
function BttleTopDramaView:updatePassCahpterInfo()
    ...
    if config then
        if self.reward_btn == nil then
            self.reward_btn = self:createSpecialIcon(PathTool.getResFrame("battledrama","battledrama_1018"), TI18N("语言_c_2475"), LOADTEXT_TYPE_PLIST)
...
--         self.rank_icon = self:createSpecialIcon(PathTool.getResFrame("battledrama", "battledrama_1015"), TI18N("语言_c_271"), LOADTEXT_TYPE_PLIST)
...
function BttleTopDramaView:updatePassCahpterVideo( )
    ...
    if self.strong_btn == nil then
        self.strong_btn = self:createSpecialIcon(PathTool.getResFrame("battledrama","battledrama_1024"), TI18N("语言_c_1364"), LOADTEXT_TYPE_PLIST)
...
--[[function BttleTopDramaView:updateEncounter( )
    ...
    if self.encounter_btn == nil then
        self.encounter_btn = self:createSpecialIcon(PathTool.getResFrame("battledrama","battledrama_1024"), TI18N("语言_c_2476"), LOADTEXT_TYPE_PLIST)
...
function BttleTopDramaView:checkEncounterOpenStatus( )
    ...
    if is_open then
        ...
        self.encounter_btn_label:setTextColor(cc.c4b(248,239,118,0xff))
        self.encounter_btn_label:enableOutline(cc.c4b(92,39,5,0xff), 2)
    else
...
function BttleTopDramaView:showWorldLevelTips( status )
    if status == true then
        if not self.world_lv_layout then
            ...
            local world_lv_title = createLabel(26,161,nil,85,world_bg_size.height-40,TI18N("语言_c_1158"),world_lv_bg,nil,cc.p(0, 0.5))
            ...
            self.world_lv_txt:setString(string.format(TI18N("语言_c_1159"), world_lev))
        end
...
function BttleTopDramaView:createDramaButton(  )
    if not _tolua_isnull(self.btn_layout) then
        if not self.battle_button_list then
            ...
            self.item = self.battle_button_list:getChildByName("item")
            local item = self.battle_button_list:getChildByName("item")
            self.descTxt = item:getChildByName("desc");
            self.descTxt:setString(TI18N("语言_c_4375"))
            self.drama_item_scrollview = item:getChildByName("item_scrollview")
            ...
            -- 掉落详细按钮
            self.detail_btn = item:getChildByName("desc")
            ...
            self.encounter_btn:addTouchEventListener(function(sender, event_type)
                ...
                if event_type == ccui.TouchEventType.ended then
                    ...
                    -- local encounter_lev = Config.EncounterData.data_encounter_const.encounter_lev
                    -- if self.role_vo and encounter_lev ~= nil and self.role_vo.lev < encounter_lev.val then 
                    --     message(_string_format(TI18N("语言_c_2478"),encounter_lev.val))
                    --     return
                    -- end
            ...
            self.quick_battle_label:setString(TI18N("语言_c_2479"))
        ...
        if not self.resour_collect then
            ...
            self.resour_collect:setPosition(-60, 118)
            self.resour_collect:setLocalZOrder(998)
                ...
                if event_type == ccui.TouchEventType.ended then
                    ...
                    if self.resources_action == PlayerAction.action then
                        ...
                        if (cur_energy+qingbao_val) > max_energy then
                            ...
                            local str = string.format(TI18N("语言_c_4773"), cur_energy, max_energy)
                            self.tips_alert = CommonAlert.show(str, TI18N("语言_c_63"), call_back, TI18N("语言_c_62"), cancel_callback, CommonAlert.type.rich,nil,nil,24)
                        else
                            requestGetAwardFunc()
                        end
                    end
        ...
        if not self.battle_button_up_list then
            self.battle_button_up_list = createCSBNote(PathTool.getTargetCSB("battle/battle_button_up_list"))
            self.battle_button_up_list:setAnchorPoint(cc.p(0.5, 0))
            local target_pos_y = MainuiController:getInstance():getBottomHeight()
            self.battle_button_up_list:setPositionY(-target_pos_y-25)
            self.battle_button_up_list:setLocalZOrder(999)
            self.btn_layout:addChild(self.battle_button_up_list)

            self.auto_btn = self.battle_button_up_list:getChildByName("auto_btn")
            self.auto_btn_label = self.auto_btn:getChildByName("label")
            self.auto_btn_label:setString(TI18N("语言_c_2480"))

            self:changeAutoState()
            self.auto_btn:addTouchEventListener(function(sender, event_type)
                customClickAction_2(sender, event_type)
                if event_type == ccui.TouchEventType.ended then
                    --local canauto = RoleController:getInstance():getModel():checkPrivilegeStatus(6)
                    -- if _drama_model:getIsBattleAutoFight() ~= 1 and not canauto then
                        -- message(TI18N("语言_c_2481"))
                    --     self:_onClickSpecialBtn(sender)
                    --     return
                    -- end
                    if _drama_model:getIsBattleAutoFight() ~= 1 then
                        _drama_model:setIsBattleAutoFight(true)
                    else
                        _drama_model:setIsBattleAutoFight(false)
                    end
                    self:changeAutoState()
                end
            end)
            if self.battle_button_list then
                ...
                if self.auto_btn then
                    local pos2 = self.auto_btn:getPositionY()
                    local node_pos1 = self.battle_button_up_list:convertToNodeSpace(pos1)
                    --self.auto_btn:setPositionY(node_pos1.y + 94 + 160 - 10)
                end
            end
        end
...
--自动挑战特权前往页面
function BttleTopDramaView:_onClickSpecialBtn( sender )
	...
	for i=1,1 do
		...
		if explain_config then
			...
			if status then
				str = str .. TI18N("语言_c_1856") --不用处理
			else
				str = str .. TI18N("语言_c_4377") .. string.format("<div href=xxx fontcolor=249003>%s</div>", TI18N("语言_c_1858")) --不用处理
			end
			if i == 1 then
				tips_str = str
			else
    ...
    local touch_posx, touch_posy = self.auto_btn:getPosition()
...
function BttleTopDramaView:_onClickGoToActivate(  )
	VipController:getInstance():openVipMainWindow(true, VIPTABCONST.PRIVILEGE,1,true)
end

function BttleTopDramaView:changeAutoState()
    if self.auto_btn then
        if _drama_model:getIsBattleAutoFight() == 0 then
            setChildUnEnabled(true, self.auto_btn)
            self.auto_btn_label:setTextColor(Config.ColorData.data_color4[1])
            self.auto_btn_label:enableOutline(cc.c4b(0x49,0x49,0x49,0xff), 2)
        else
            setChildUnEnabled(false, self.auto_btn)
            self.auto_btn_label:setTextColor(cc.c4b(0xff,0xf4,0xc8,0xff))
            self.auto_btn_label:enableOutline(cc.c4b(0x4f,0x16,0x00,0xff), 2)
        end
    end
end
    ...
    if not _tolua_isnull(self.next_battle_btn) and not _tolua_isnull(self.next_battle_time) then
        ...
        if data.status == 2 and data.cool_time == 0 then            --可挑战
            ...
            if config and config.lev_limit and config.lev_limit > role_vo_lev then
                ...
                self.boss_btn_notice:setString(string.format(TI18N("语言_c_2482"), config.lev_limit))
            else
                self.boss_btn_notice:setVisible(false)
                self.boss_btn_challenge:setVisible(true)
            end
        ...
        elseif data.status == 3 then                                --已通过
            ...
            self.boss_btn_notice:setVisible(true)
            self.boss_btn_challenge:setVisible(false)
            self.boss_btn_notice:setString(TI18N("语言_c_2483"))
        end
        -- 保存一下挑战按钮的状态
    ...
    if not _tolua_isnull(self.quick_battle_btn) then
        ...
        if self.quick_battle_status ~= is_open then
            ...
            if is_open then
                ...
                self.quick_battle_label:setTextColor(cc.c4b(248,239,118,0xff))
                self.quick_battle_label:enableOutline(cc.c4b(92,39,5,0xff), 2)
            else
...
function BttleTopDramaView:TurnMoveFun()
    -- self.drama_item_scrollview:scrollToRight(5,false)
    local cont = self.drama_item_scrollview:getInnerContainer()
    local sWidth = self.drama_item_scrollview:getContentSize().width
    local contWidth = cont:getContentSize().width
    local a = cont:getPositionY()
    doStopAllActions(cont)
    local temp = cc.p(sWidth- contWidth,a)
    if not self.left_or_right then 
        temp = cc.p(0,a)
    end
    local moveto = cc.MoveTo:create(10,temp)
    local action = cc.Sequence:create(moveto,cc.CallFunc:create(function()
        self.left_or_right = not self.left_or_right
        self:TurnMoveFun()
    end))
    cont:runAction(action)
end
...
function BttleTopDramaView:updateBtnLayerStatus( status )
    ...
    if self.next_battle_btn then
        ...
        if status == true then
            self.boss_btn_challenge:getChildByName("label"):setString(TI18N("语言_c_1884"))
            setChildUnEnabled(false, self.next_battle_btn)
            self.boss_btn_challenge_label:setTextColor(cc.c4b(248,239,118,0xff))
            self.boss_btn_challenge_label:enableOutline(cc.c4b(92,39,5,0xff), 2)
        else
            setChildUnEnabled(true, self.next_battle_btn)
            self.boss_btn_challenge_label:setTextColor(Config.ColorData.data_color4[1])
            self.boss_btn_challenge_label:enableOutline(cc.c4b(0x49,0x49,0x49,0xff), 2)
            self.boss_btn_challenge:getChildByName("label"):setString(TI18N("语言_c_2484"))
        end
    end
end
    ...
    if status == true then
        ...
        if self.btn_layout and not _tolua_isnull(self.btn_layout) then
            if _tolua_isnull(self.form_view) then
                ...
                self.form_view:setPosition(0, 74)
                if not _tolua_isnull(self.battle_button_list) then
                    self.battle_button_list:getChildByName("image_bg"):getChildByName("image_bg_4"):setVisible(false)
                    self.battle_button_list:getChildByName("image_bg"):getChildByName("image_bg_7"):setVisible(false)
                    self.battle_button_list:getChildByName("reward_layout"):setVisible(false)
                end
                ...
                self.buff_btn:setPositionY(82 + 15)
            ...
            if not _tolua_isnull(self.left_camp_btn) then
                ...
                local halo_icon_config, alien_effect = BattleController:getInstance():getModel():getCampIconConfigByIds(left_halo_id_list)
                if halo_icon_config and halo_icon_config.icon then
                    ...
                    if not self.left_camp_effect then
                        ...
                        self.left_camp_effect:setCascadeOpacityEnabled(true)
                    end
                    if alien_effect and alien_effect ~= "" and not self.left_camp_effect.alien_effect then
                        local size = self.left_camp_effect:getContentSize()
                        local alien_res = PathTool.getCampGroupIcon("2001")
                        self.left_camp_effect.alien_effect = createImage(self.left_camp_effect, "", size.width/2, size.height/2, cc.p(0.5, 0.5), false)
                        loadImageTextureFromCDN(self.left_camp_effect.alien_effect, alien_res, ResourcesType.single)
                    end
                    if self.left_camp_effect.alien_effect then
                        self.left_camp_effect.alien_effect:setVisible(alien_effect and alien_effect ~= "")
                    end
            ...
            if not _tolua_isnull(self.right_camp_btn) then
                local halo_res = PathTool.getCampGroupIcon( 1000 )
                local halo_icon_config, alien_effect = BattleController:getInstance():getModel():getCampIconConfigByIds(right_halo_id_list)
                if halo_icon_config and halo_icon_config.icon then
                    halo_res = PathTool.getCampGroupIcon(halo_icon_config.icon)
                    if not self.right_camp_effect then
                        ...
                        self.right_camp_effect:setCascadeOpacityEnabled(true)
                    end
                    if alien_effect and alien_effect ~= "" and not self.right_camp_effect.alien_effect then
                        local size = self.right_camp_effect:getContentSize()
                        local alien_res = PathTool.getCampGroupIcon("2001")
                        self.right_camp_effect.alien_effect = createImage(self.right_camp_effect, "", size.width/2, size.height/2, cc.p(0.5, 0.5), false)
                        loadImageTextureFromCDN(self.right_camp_effect.alien_effect, alien_res, ResourcesType.single)
                    end
                    if self.right_camp_effect.alien_effect then
                        self.right_camp_effect.alien_effect:setVisible(alien_effect and alien_effect ~= "")
                    end
        ...
        if self.form_view and not _tolua_isnull(self.form_view) then
            ...
            if not _tolua_isnull(self.battle_button_list) then
                self.battle_button_list:getChildByName("image_bg"):getChildByName("image_bg_4"):setVisible(true)
                self.battle_button_list:getChildByName("image_bg"):getChildByName("image_bg_7"):setVisible(true)
                self.battle_button_list:getChildByName("reward_layout"):setVisible(true)
            end
        end
    end
end
...
function BttleTopDramaView:updateRound(round)
    ...
    if not _tolua_isnull(self.round_label) then
        self.round_label:setString(string.format(TI18N("语言_c_2485"), round, total_round))
    end
end
...
--重新处理 自动挑战，远航按钮 章节掉落列表 位置
function BttleTopDramaView:resetButtonPosition( ... )
    if self.battle_button_list then
        local pos1 = self.battle_button_list:getChildByName("image_bg"):convertToWorldSpace(cc.p(0, 0))
        if self.auto_btn then
            local pos2 = self.auto_btn:getPositionY()
