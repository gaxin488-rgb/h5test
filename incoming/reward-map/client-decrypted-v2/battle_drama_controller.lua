...
function BattleDramaController:send13003(is_auto)
    if is_auto ~= 2 then
        if self.model:getIsBattleAutoFight() == 1 then
            is_auto = 1
        else
            is_auto = 0
        end
...
function BattleDramaController:dramaCanShowMVP()
    if self.model and self.model:getIsBattleAutoFight() == 1 then
        return false
        -- if self.battle_drama_ui and not tolua.isnull(self.battle_drama_ui) then
        --     if self.battle_drama_ui.is_showing ~= true then
        --         return false
        --     end
        -- else
        --     return false
...
function BattleDramaController:handle13018(data)
    AudioManager:getInstance():playEffect(AudioManager.AUDIO_TYPE.COMMON, "c_coin")
    message(data.msg)
end
...
function BattleDramaController:handle13019(data)
    ...
    local confirm_str = TI18N("语言_c_108")
    local desc = TI18N("语言_c_2526")
    if main_index == MainuiConst.btn_index.drama_scene then
        confirm_str = TI18N("语言_c_63")
        desc = TI18N("语言_c_2527")
    end
    ...
    self.hook_alert = CommonAlert.show(desc, confirm_str, confirm_callback, TI18N("语言_c_62"), cancel_callback, nil, nil, {view_tag=ViewMgrTag.RECONNECT_TAG})
end
