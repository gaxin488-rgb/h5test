...
RollerWishWindow = RollerWishWindow or BaseClass(BaseView)

function RollerWishWindow:__init()
    ...
    local lucky_sprite_num_cfg = Config.HolidayProhibitedScrollData.data_const["lucky_prohibitor_num"]
    if lucky_sprite_num_cfg then
        self.lucky_sprite_num = lucky_sprite_num_cfg.val -- 可许愿数量
    end
end

function RollerWishWindow:open_callback()
    ...
    self.win_title:setString(TI18N("语言_c_7370"))
    ...
    self.choose_tips_txt:setPositionY(615)
    container:addChild(self.choose_tips_txt)
    self.choose_tips_txt:setString(TI18N("语言_c_7371"))
    -- self.choose_tips_txt:setString(transformTextToShort(TI18N("语言_c_7371"), 150))
    -- addEvt2showAllTextTips(self.choose_tips_txt, TI18N("语言_c_6944"), 150, nil, nil, nil, true, true)
end

function RollerWishWindow:createNewCell()
    local cell = RollerWishItem.new()
...
function RollerWishWindow:numberOfCells()
    if not self.elfin_data_list then
        return 0
    end
    return #self.elfin_data_list
end

function RollerWishWindow:updateCellByIndex(cell, index)
    cell.index = index
...
function RollerWishWindow:onCellTouched(cell)
...
function RollerWishWindow:updateElfinInfo()
    if not self.cur_item_id_list or #self.cur_item_id_list <= 0 then -- 没选中任何一个精灵
        self.name_txt:setString(TI18N("语言_c_1940"))
        local bwidth = self.name_txt:getContentSize().width + 40
        self.Image_1:setContentSize(cc.size(bwidth,37))
   ...
   -- print("self.item_scrollview-------------",vardump(data_list))
    if data_list and next(data_list) then
        local item_id = data_list[1][1]
        local cur_cfg = Config.ItemData.data_get_data(item_id)
        self.name_txt:setTouchEnabled(true)
        transformTextToShortByWidth(self.name_txt, cur_cfg.name, 250)
        -- if self.name_txt then
        addEvt2showAllTextTips(self.name_txt,cur_cfg.name, 10)
        -- end
        autoSizeTitleBg(self.name_txt, self.Image_1, 20)
    else
        self.name_txt:setString(TI18N("语言_c_1940"))
        autoSizeTitleBg(self.name_txt, self.Image_1, 20)
    end
...
function RollerWishWindow:register_event()
    ...
    registerButtonEventListener(self.btn_rule, function()
        if self.summon_data then
            local action_cfg = Config.HolidayProhibitedScrollData.data_action[self.summon_data.camp_id]
            if action_cfg and action_cfg.group_id then
                local conf = Config.HolidayProhibitedScrollData.data_summon[action_cfg.group_id]
                if conf then
                    TimesummonController:getInstance():openTimeSummonAwardView(true, conf.group_id, self.summon_data,TimesummonConst.ActonInfoType.Roller)
                end
            end
        end
    end, true, 1)
...
function RollerWishWindow:onClickCloseBtn()
    _controller:openRollerWishWindow(false)
end

function RollerWishWindow:onClickChooseBtn()
        ...
        for k, v in pairs(self.cur_item_id_list) do
            if v then
                _table_insert(temp_list, {
                    lucky_prohs_bid = v.id
                })
            end
        end
        _controller:sender20835(temp_list)
        self:onClickCloseBtn()
    end
end

function RollerWishWindow:showCommonAlert(is_close)
    local str = TI18N("语言_c_7373")
...
function RollerWishWindow:setData()
    local data = _model:getRollerSummonData()
    if not data then
        return
    end
    ...
    for k, v in pairs(data.lucky_ids) do
        if v.lucky_prohs_bid then
            _table_insert(self.elfin_bid_list, v.lucky_prohs_bid)
        end
    end

    self.elfin_data_list = {}
    local all_elfin_data = {}
    local lucky_sprite_ids_cfg = Config.HolidayProhibitedScrollData.data_const["lucky_prohibitor_ids"]
...
function RollerWishWindow:updateIconShow()
...
function RollerWishWindow:openRootWnd()
    self:setData()
end

function RollerWishWindow:close_callback()
...
--------------------------@ item
RollerWishItem = class("RollerWishItem", function()
    return ccui.Widget:create()
end)

function RollerWishItem:ctor()
    self:configUI()
	-- self:registerEvent()
end

function RollerWishItem:configUI()
...
function RollerWishItem:addCallBack(callback)
    if self.item_node then
        self.item_node:addCallBack(callback)
    end
end

function RollerWishItem:setData(data)
	-- self.data = data
	-- print(vardump(data))
    if self.item_node then
...
function RollerWishItem:IsGetStatus(status)
    if self.item_node then
        self.item_node:IsGetStatus(status)
    end
end
-- function RollerWishItem:setLongTimeTouchEffect(is_touch)
--     self.have_long_time_effect = is_touch
-- end

function RollerWishItem:DeleteMe()
    if self.item_node then
        self.item_node:DeleteMe()
        self.item_node = nil
    end
