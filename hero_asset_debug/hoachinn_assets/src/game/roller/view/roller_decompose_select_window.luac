--卷轴碎片分解 选择弹窗 --------------------------------------------------------------------
-- --------------------------------------------------------------------
RollerDecomposeSelectWindow = RollerDecomposeSelectWindow or BaseClass(BaseView)

function RollerDecomposeSelectWindow:__init()
    self.ctrl = RollerController:getInstance()
...
function RollerDecomposeSelectWindow:open_callback()
...
function RollerDecomposeSelectWindow:register_event()
    ...
    registerButtonEventListener(self.btn_comp, function()
        if self.cur_number then
            GlobalEvent:getInstance():Fire(RollerEvent.RollerFenjieSelectNumEvent ,{id = self.data.id,item_id = self.data.base_id,num = self.cur_number})
            self:close()
        end
    end,true, 1)
end
function RollerDecomposeSelectWindow:touch_redu()
...
function RollerDecomposeSelectWindow:touch_add()
    self.cur_number = self.cur_number + 1
    if self.cur_number >= imput_number then
        self:setTouchEnable_Add(true)
...
function RollerDecomposeSelectWindow:setTouchEnable_Add(bool)
    setChildUnEnabled(bool,self.btn_add)
    self.btn_add:setTouchEnabled(not bool)
end
function RollerDecomposeSelectWindow:setTouchEnable_Redu(bool)
    setChildUnEnabled(bool,self.btn_redu)
    self.btn_redu:setTouchEnabled(not bool)
end

function RollerDecomposeSelectWindow:openRootWnd(data)
...
function RollerDecomposeSelectWindow:setBaseInfo()
    ...
    local item_data = self.data.quantity --BackpackController:getModel():getPackItemNumByBid(1,self.item_config.id)
    if item_data > 0 then
        self.cur_number = 1
    else
        self.cur_number = 0
...
function RollerDecomposeSelectWindow:close_callback()
    if self.goods_item then 
        self.goods_item:DeleteMe()
    end
    self.goods_item = nil
    self.ctrl:openJinqiFenjieSelectNumWindow(false)
end
