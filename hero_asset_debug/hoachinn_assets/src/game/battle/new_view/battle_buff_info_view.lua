...
function BattleBuffInfoView:open_callback(  )
	...
	self.left_name_label:ignoreContentAdaptWithSize(true)
	self.left_name_label:setTextAreaSize(cc.size(280, 0))
	setTextSpacing(self.left_name_label)
	self.right_name_label = container:getChildByName("right_name_label")
	self.right_name_label:ignoreContentAdaptWithSize(true)
	self.right_name_label:setTextAreaSize(cc.size(280, 0))
	setTextSpacing(self.right_name_label)
    ...
    close_btn_label:setString(TI18N("语言_c_4217"))
    
    local title_label = container:getChildByName("title_label")
    title_label:setString(TI18N("语言_c_4218"))
end
...
function BattleBuffInfoView:updateName(left_name, right_name)
...
function BattleBuffInfoView:openRootWnd( left_name, right_name, battle_type)
	self.left_name = left_name or ""
	self.right_name = right_name or ""
	self.battle_type = battle_type or nil
	self:setData(true)
end
...
function BattleBuffInfoView:setData( is_init )
	...
	for k,bRole in pairs(all_object) do
		-- 筛选掉神器
		if bRole.object_type == BattleObjectType.Pet or (bRole.object_type == BattleObjectType.Unit and not (bRole.hp <= 0 or bRole.is_die)) then
			...
			delayRun(
	            self.left_role_panel, i*4 / display.DEFAULT_FPS, function()
					...
					if role_item == nil then
						role_item = BattleBuffInfoItem.new(Dir_Type.Left,i,self.battle_type)
		...
		if is_init then
			delayRun(
	            self.right_role_panel, i*4 / display.DEFAULT_FPS, function()
					...
					if role_item == nil then
						role_item = BattleBuffInfoItem.new(Dir_Type.Right,i,self.battle_type)
...
function BattleBuffInfoItem:ctor(dir, i,battle_type)
...
function BattleBuffInfoItem:setData( data )
    ...
    for i,v in pairs(data.buff_list) do
		...
		if res_id ~= nil and res_id ~= 0 and (self.data.hp > 0 or (buff_config.clean_when_dead and buff_config.clean_when_dead == "false")) then
			if temp_list[res_id] == nil then
				temp_list[res_id] = {res_id=res_id, num=0, name=buff_config.name, remain_round=v.remain_round, add_count = v.add_count, desc=buff_config.desc, buff_infos = {}}
			end
			if temp_list[res_id].num == 0 or (buff_config.join_type and buff_config.join_type ~= 3) then
				...
				_table_insert(temp_list[res_id].buff_infos, {buff_id=v.bid, remain_round=v.remain_round, add_count = v.add_count, group = buff_config.group, group_pro=buff_config.group_pro})
	...
	if data.is_die or data.hp <= 0 then
		if next(self.buff_list_data) then
			self.hero_head:showStrTips(false)
		else
			self.hero_head:showStrTips(true,TI18N("语言_c_70"),{c3b = cc.c3b(255,255,255)})
		end
	else
		self.hero_head:showStrTips(false)
	end
