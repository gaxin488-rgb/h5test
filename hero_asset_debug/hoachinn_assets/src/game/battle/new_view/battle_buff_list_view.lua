...
function BattleBuffListItem:setData( data )
	...
	local total_h = 40 + self.round_label:getContentSize().height + 3
	...
	if not data.remain_round or data.remain_round > 30 then
		...
		local _txt = TI18N("语言_c_4219")
		local _num = data.remain_round
		local cur_round = _model:getFightActionCount()
		if data.add_count and cur_round then
			_num = math.max(data.remain_round - (cur_round - data.add_count), 0)
		else
			_num = data.remain_round
		end
		local buff_id =  data.buff_id
		local skill_cfg = Config.SkillData.data_get_buff
		if skill_cfg and skill_cfg[buff_id] and skill_cfg[buff_id].buff_result_type == "13" then
			_txt = TI18N("语言_c_7003")
		end
		self.round_label:setString(string.format(_txt, _num))
		total_h = total_h + self.round_label:getContentSize().height
	end
	-- 描述
	if not self.buff_desc then
		local y = self.name_label:getPositionY() - self.name_label:getContentSize().height - 3
		if self.round_label:isVisible() then
			y = y - self.round_label:getContentSize().height - 3
		end
		self.buff_desc = createRichLabel(22, cc.c3b(224, 191, 152), cc.p(0, 1), cc.p(30, y), 2, nil, 500)
		self.container:addChild(self.buff_desc)
	end
	self.buff_desc:setString(data.desc or "")
	total_h = total_h + self.buff_desc:getContentSize().height + 14
	self.size.height = total_h
	self:setContentSize(self.size)
	self.root_wnd:setContentSize(self.size)
	self.root_wnd:setPosition(0, 0)
	ccui.Helper:doLayout(self.root_wnd)
	if self.round_label:isVisible() then
		self.round_label:setPositionY(self.name_label:getPositionY() - self.name_label:getContentSize().height - 3)
		self.buff_desc:setPositionY(self.name_label:getPositionY() - self.name_label:getContentSize().height - 3 - self.round_label:getContentSize().height - 3)
	else
		self.buff_desc:setPositionY(self.name_label:getPositionY() - self.name_label:getContentSize().height - 3)
	end
end

function BattleBuffListItem:getSize()
	return self.size
end
