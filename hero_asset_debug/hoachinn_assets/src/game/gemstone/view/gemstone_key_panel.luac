...
-- @description    : 
        -- 显示解锁家园钥匙的弹窗
---------------------------------
GemstoneKeyPanel = GemstoneKeyPanel or BaseClass(BaseView)

local controller = GemstoneController:getInstance()

function GemstoneKeyPanel:__init()
...
function GemstoneKeyPanel:open_callback(  )
    ...
    self.icon = icon
    local bg_res = PathTool.getPlistImgForDownLoad("homeworld", "homeworld_unlock_gem")
    ...
    self.share_btn:getChildByName("label"):setString(TI18N("语言_c_7190"))
    --引导需要
...
function GemstoneKeyPanel:register_event(  )
    registerButtonEventListener(self.background, function()
        -- controller:openGemstoneKeyPanel(false)
    end,false, 2)
    registerButtonEventListener(self.share_btn, function(param, sender) self:onShareBtn(sender)  end ,true, 1)
end

--跳转家园
function GemstoneKeyPanel:onShareBtn(sender)
    controller:openGemstoneKeyPanel(false)
    JumpController:getInstance():jumpViewByEvtData({777})
end



function GemstoneKeyPanel:openRootWnd(setting)
    ...
    self.name:setString(TI18N("语言_c_7114"))

    self:handleEffect(true)
    self.desc:setString(string.format(TI18N("语言_c_7191")))
...
function GemstoneKeyPanel:runmoveAction(node, delay)
    if delay <= 0 then
        delay = 1
    end
...
function GemstoneKeyPanel:handleEffect(status)
...
function GemstoneKeyPanel:close_callback()
