...
function HeroLibraryInfoWindow:open_callback()
    ...
    local attr_btn_label = self.attr_btn:getChildByName("label")
    attr_btn_label:setString(TI18N("语言_c_3075"))
    setTextSpacing(attr_btn_label,-15)
    setTextMaxWidth(attr_btn_label,105)
    --查看传记
    self.story_btn =  self.box_6:getChildByName("story_btn")
    local story_btn_lebel = self.story_btn:getChildByName("label")
    story_btn_lebel:setString(TI18N("语言_c_3076"))
    setTextSpacing(story_btn_lebel,-15)
    setTextMaxWidth(story_btn_lebel,105)

    --分享英雄
    self.share_btn =  self.box_6:getChildByName("share_btn")
    local share_btn_label = self.share_btn:getChildByName("label")
    share_btn_label:setString(TI18N("语言_c_3077"))
    setTextSpacing(share_btn_label,-15)
    setTextMaxWidth(share_btn_label,105)
    if IS_SHOW_GAME_SHARE_HERO then
    ...
    self.encounter_btn:getChildByName("label"):setString(TI18N("语言_c_1349"))
...
--     self.skill_panel =  self.box_6:getChildByName("skill_panel")
    ...
    -- self.voice_btn:getChildByName("label"):setString(TI18N("语言_c_3078"))
    -- --左右按钮
...
function HeroLibraryInfoWindow:onClickVoiceBtn()
    ...
    if self.partner_config.voice and self.partner_config.voice ~= "" then
        ...
        self.hero_music = controller:onPlayHeroVoice(voice, time)
    end
end

function HeroLibraryInfoWindow:stopHeroVoice()
    if self.hero_music then
        HeroController:getInstance():stopHeroVoice(self.hero_music)
        self.hero_music = nil
    end
end
...
function HeroLibraryInfoWindow:openRootWnd(bid, library_config)
    ...
    if self.partner_config.voice and self.partner_config.voice ~= "" then
        local voice = self.partner_config.voice 
        controller:preloadHeroVoice(voice)
    end
    ...
    addRedPointToNodeByStatus(self.story_btn,model:getHeroLibRed(bid))
    --请求是否可显示冒险奇遇
...
function HeroLibraryInfoWindow:updateHeroInfo()
    ...
    self.hero_name_bg:setContentSize(self.hero_name:getContentSize().width + 40,42)
    self.hero_name:setPositionX(self.hero_name_bg:getContentSize().width/2)
    local score = HeroModel:heroBidScore(self.partner_config.bid)
    ...
    self.title_bg:setPositionX(self.hero_name_bg:getContentSize().width/2)
    self.title_name:setPositionX(self.hero_name_bg:getContentSize().width/2)
end

--创建page
function HeroLibraryInfoWindow:createSkillPanel(layout)
    if not self.partner_config or not self.library_config then return end
    local attr_name_str = string_format("%s：%s%s",TI18N("语言_c_836"), tostring(HeroConst.CampAttrName[self.partner_config.camp_type]), tostring(HeroConst.CareerName[self.partner_config.type]))
    local label1= createLabel(26, cc.c4b(0x64,0x32,0x23,0xff), nil, 70, 220, attr_name_str, layout, FONT_COMMON_LINE_SPACE, cc.p(0,1))
    label1:setWidth(540)

    local type_name_str = string_format("%s：%s",TI18N("语言_c_3080"), self.partner_config.hero_pos)
    local str = createLabel(26, cc.c4b(0x64,0x32,0x23,0xff), nil, 70, 190, type_name_str, layout,FONT_COMMON_LINE_SPACE, cc.p(0,1))
    str:setWidth(540)
    -- local strWidth = str:getContentSize().width
    -- local strX = 700 - strWidth - 10
    -- str:setPositionX(strX)
...
function HeroLibraryInfoWindow:createVoicePanel(layout)
    ...
    local val = createRichLabel(26, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0, 0.5), cc.p(70,130), 0, nil, 580)
    layout:addChild(val)
    ...
    if self.partner_config.voice and self.partner_config.voice ~= "" then
        ...
        createLabel(22, cc.c4b(0xf5,0xf3,0xe9,0xff), cc.c4b(0x46,0x29,0x0a,0xff), 40, 23, TI18N("语言_c_3078"), btn_layout, 2, cc.p(0,0.5))
    end
end
...
function HeroLibraryInfoWindow:showVoicePanel(status)
    if status then
        ...
        if self.is_init_voice == nil then
            ...
            local val = createRichLabel(26, cc.c4b(0x64,0x32,0x23,0xff), cc.p(0, 1), cc.p(0,0), 0, nil, 580)
...
function HeroLibraryInfoWindow:initSkill()
    ...
    for i,skill in ipairs(skill_list) do
        ...
        if config then
            ...
            self.skill_item_list[i]:setSkillSubscript(Config.PartnerData.data_partner_base[self.partner_config.bid].hurt_type)
            self.skill_item_list[i]:setPosition( x + item_width * (i - 1) + item_width * 0.5, 130*0.5-1)
        else 
