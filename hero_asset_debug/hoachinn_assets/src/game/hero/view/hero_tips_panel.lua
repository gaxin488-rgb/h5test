...
function HeroTipsPanel:open_callback()
    ...
    self.sp_career = self.top_panel:getChildByName("sp_career")
    --战力    
    ...
    local advancedKey = self.top_panel:getChildByName("advancedKey")
    advancedKey:setString(TI18N("语言_c_855"))
    self.advanced_node:setPositionX(advancedKey:getPositionX() + advancedKey:getContentSize().width + 10)
...
function HeroTipsPanel:updateScoreImg()
    local cfg = Config.PartnerData.data_partner_base[self.hero_vo.bid]
    local score = cfg.score
    local skin_cfg = Config.PartnerSkinData.data_partner_bid_info[self.hero_vo.bid]
    if skin_cfg then
        for k, v in pairs(skin_cfg) do
            if LookController:getInstance():getModel():getIsHaveSkin(self.hero_vo.bid,v.skin_id) then
    ...
    LookController:getInstance():getModel().hero_skin_list = nil
    self.hero_name:setPositionX(self.score_image:getPositionX()+self.score_image:getContentSize().width*self.score_image:getScale()*0.5)
end
...
function HeroTipsPanel:initSkill(hero_vo, break_config)
    ...
    for i,skill in ipairs(skill_list) do
        ...
        if config then
            ...
            self.skill_item_list[i]:setData(config,{is_sp=true})
...
function HeroTipsPanel:updateEquip(hero_vo)
    ...
    if hero_vo.eqms ~= nil then
        ...
        for i,v in ipairs(hero_vo.eqms) do
            ...
            -- if v.gold_eqm_info and next(v.gold_eqm_info) then
    ...
    if #skill_list > 0 then
        ...
        for i,v in ipairs(skill_list) do
            ...
            if config then
                ...
                self.talent_skill_item_list[i]:setData(config,{is_sp=true})
...
function HeroTipsPanel:updateHolyEquip(hero_vo)
    ...
    for i,equip_node in ipairs(self.holy_equip_node_list or {}) do
