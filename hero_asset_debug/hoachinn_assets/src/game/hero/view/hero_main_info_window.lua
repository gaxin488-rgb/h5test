...
function HeroMainInfoWindow:__init()
    ...
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("hero", "special_hero_star"), type = ResourcesType.plist},
        ...
        {path = PathTool.getPlistImgForDownLoad("hero","special_hero_nin_bg"), type = ResourcesType.single},
        {path = PathTool.getPlistImgForDownLoad("hero","special_hero_tai_bg"), type = ResourcesType.single},
        {path = PathTool.getPlistImgForDownLoad("hero","txt_cn_hero_star_ur"), type = ResourcesType.single},
        {path = PathTool.getPlistImgForDownLoad("hero","txt_cn_ur_awaken_btn"), type = ResourcesType.single},
...
function HeroMainInfoWindow:open_callback()
    ...
    self.starup_help_btn = self.main_panel:getChildByName("starup_help_btn")
    self.starup_help_btn:setVisible(true)
    ...
    self.fight_label:setPosition(103, 32) 
    ...
    self.score_image = self.main_container:getChildByName("score_image")
    ...
    for i=1,5 do
        ...
        local label_panel = item:getChildByName("label_panel")
        tab_btn.title = label_panel:getChildByName("label")
        tab_btn.title:setTextColor(cc.c4b(249,238,208,255))
        tab_btn.title:enableOutline(cc.c4b(92,39,5,255), 2)
        tab_btn.title:ignoreContentAdaptWithSize(true)
        tab_btn.title:setTextAreaSize(cc.size(120, 0))
        setTextSpacing(tab_btn.title, -12)
...
function HeroMainInfoWindow:register_event()
    ...
    registerButtonEventListener(self.starup_help_btn, function() self:onClickStarupHelpBtn() end ,true, 1)
    ...
    --已穿戴装备合成
    self:addGlobalEvent(HeroEvent.Equip_Compose_Event, function()
    ...
    self:addGlobalEvent(HeroEvent.Hero_Update_Star_Close_Event, function(hero_vo)
        ...
        if hero_vo and hero_vo.partner_id == self.select_hero_vo.partner_id then
            ...
            if self.select_hero_vo.star >= 11 then
                self:updatePageInfo()
            end
        end
    end)
    ...
    local function onTouchBegin(touch, event)
        ...
        if not GuideController:getInstance():isInGuide() then
    ...
    local function onTouchEnded(touch, event)
        ...
        if self.touch_pointX then
            ...
            elseif moveX < -100 then
                if self.select_index ~= #self.hero_list then
...
--升星消耗预览
function HeroMainInfoWindow:onClickStarupHelpBtn()
    controller:openHeroStarupCostHelpPanel(true, self.select_hero_vo)
end
...
--大神推荐
function HeroMainInfoWindow:onClickRecommendBtn()
    controller:openHeroCollocationWindow(true, self.select_hero_vo)
end
--分享
...
function HeroMainInfoWindow:showSharePanel(bool)
    ...
    if not self.share_panel then 
        ...
        local btn1 =  createButton(self.share_panel, TI18N("语言_c_2994"), size.width/2, 115, cc.size(160,64), res, 26, Config.ColorData.data_color4[1])
        ...
        local btn2 =  createButton(self.share_panel, TI18N("语言_c_2995"), size.width/2, 45, cc.size(160,64), res, 26, Config.ColorData.data_color4[1])
        ...
        local btn3 =  createButton(self.share_panel, TI18N("语言_c_2996"), size.width/2, 185, cc.size(160,64), res, 26, Config.ColorData.data_color4[1])
...
function HeroMainInfoWindow:updateTabBtnZOrder()
...
function HeroMainInfoWindow:changeTabType(index, check_repeat_click, sender)
    ...
    if self.cur_tab ~= nil then
        self.cur_tab.title:setTextColor(cc.c4b(249,238,208,255))
        self.cur_tab.title:enableOutline(cc.c4b(92,39,5,255), 2)
    ...
    if self.cur_tab ~= nil then
        ...
        self.cur_tab.title:setTextColor(cc.c4b(248,239,118,255))
        self.cur_tab.title:enableOutline(cc.c4b(92,39,5,255), 2)
        self.cur_tab.select_bg:setVisible(true)
        self.cur_tab.btn:setLocalZOrder(9)
        self.cur_tab.title:setFontSize(26)
    end
...
function HeroMainInfoWindow:createSubPanel(index)
    ...
    if panel == nil then
        ...
        elseif index == HeroConst.MainInfoTab.eMainSoulHomology then --灵魂同调
            panel = HeroMainTabSoulHomologyPanel.new()
        end
...
function HeroMainInfoWindow:showLevelUpAction()
    if self.effect_parent == nil then
        ...
        for i=1,4 do
            ...
            local text_content = TI18N("语言_c_4555") 
...
function HeroMainInfoWindow:updateHeadHeroInfo()
    ...
    local score = model:heroBidScore(self.select_hero_vo.bid)
    ...
    if self.select_hero_vo.name then
        self.hero_name:setString(self.select_hero_vo.name)
    end

    -- local score = self.select_hero_vo.score
    ...
    --是否显示大神推荐按钮
    if score and score >= 7 and role_vo.lev >= 60 then
        -- self.comment_btn:setVisible(false)
        self.comment_btn:setVisible(false)
        --self.recommend_btn:setVisible(true)
        if not self.spine_model then
            self.spine_model = createEffectSpine("E50104",cc.p(33,35),cc.p(0.5,0.5),true,PlayerAction.action_1)
            self.spine_model:setScale(0.5)
           -- self.recommend_btn:addChild(self.spine_model)
        end
    else
        self.comment_btn:setVisible(true)
        --self.recommend_btn:setVisible(false)
    ...
    --升星助手
    local config = Config.PartnerData.data_partner_base[self.select_hero_vo.bid]
    local max_star = Config.PartnerData.data_partner_max_star[self.select_hero_vo.bid]
    self.starup_help_btn:setVisible(config.init_star < max_star and self.select_hero_vo.camp_type ~= HeroConst.CampType.eAlien)
    -- 提审状态下不显示立绘、音效按钮
...
function HeroMainInfoWindow:updateHeroBg()
    ...
    if skin_config and skin_config.hero_info_bg_res and skin_config.hero_info_bg_res ~= "" then
        ...
        local score = model:heroBidScore(self.select_hero_vo.bid)
        if score == 9 then
            bg_res = PathTool.getPlistImgForDownLoad("bigbg/hero", HeroConst.SpCampBgRes[camp_type], true)
        else
...
function HeroMainInfoWindow:updatePageInfo()
    if self.show_model_type == HeroConst.BagTab.eBagHero then
        ...
        --灵魂同调
        if self:checkSoulHomologyTabShow() then
            table_insert(self.tab_type_list, HeroConst.MainInfoTab.eMainSoulHomology)
        end
        for i,tab_btn in ipairs(self.tab_list) do
            if self.tab_type_list[i] then
                ...
                elseif tab_btn.type == HeroConst.MainInfoTab.eMainHolyequipment and self.is_holy_equp_lock then
                    ...
                    tab_btn.title:setTextColor(cc.c4b(205, 200, 177, 255))
                    tab_btn.title:enableOutline(cc.c4b(0, 0, 0, 255), 2)
                end
...
function HeroMainInfoWindow:checkStarTabShow(star)
    ...
    if self.select_hero_vo.camp_type == HeroConst.CampType.eAlien then --异界英雄只能通过同调升星
        return false
    end
    -- 熔炼祭坛 有的也要出现
...
function HeroMainInfoWindow:updateStarLockInfo(star)
    self.star_lock_dec = TI18N("语言_c_1397")
    if star == model.hero_info_upgrade_star_param2 then
        ...
        local is_show = true -- self:checkStartByConfig(config, config2)
...
--检查灵魂同调出现逻辑
function HeroMainInfoWindow:checkSoulHomologyTabShow()
    if self.select_hero_vo.camp_type == HeroConst.CampType.eAlien and model:checkSoulHeroIsOpen() then
        return true
    end
    return false
end
