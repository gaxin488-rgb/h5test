...
function FormGoFightPanel:open_callback()
    ...
    self.title:setString(TI18N("语言_c_3144"))

    self.txt_cn_common_notice_1 = self.main_container:getChildByName("txt_cn_common_notice_1")

    -- self.main_container:getChildByName("exit_tips_label"):setString(TI18N("语言_c_1837"))
    ...
    self.camp_btn_list[HeroConst.CampType.eAlien]  = camp_node:getChildByName("camp_btn6")
    ...
    self.no_vedio_label:setString(TI18N("语言_c_172"))
    ...
    self.fight_btn:getChildByName("label"):setString(TI18N("语言_c_3150"))
    --保存布阵
    ...
    self.key_up_btn:getChildByName("label"):setString(TI18N("语言_c_3151"))
     --阵法icon
...
function FormGoFightPanel:register_event()
    ...
    registerButtonEventListener(self.fight_btn, handler(self, self.onClickFightBtn) ,true, 1)
    ...
    registerButtonEventListener(self.key_up_btn, handler(self, self.onClickKeyUpBtn) ,true, 1)
    registerButtonEventListener(self.save_btn, handler(self, self.onClickSaveBtn) ,true, 1)
    ...
    if self.team_tab_list then
        for i,v in ipairs(self.team_tab_list) do
            registerButtonEventListener(v.tab_btn, function() self:onClickTeamTabBtn(v.index, true) end ,false, 1)
        end
    end

    --阵营按钮
    for select_camp, v in pairs(self.camp_btn_list) do
        registerButtonEventListener(v, function() self:onClickBtnShowByIndex(select_camp) end ,true, 1)
    end
            ...
            if event_type == ccui.TouchEventType.began then
                ...
                delayRun(self.form_panel, 0.6, function ()
                    if self.long_touch_type == LONG_TOUCH_BEGAN_TYPE then
                        if not self.five_hero_vo or not self.five_hero_vo[self.select_team_index] or not self.five_hero_vo[self.select_team_index][i] then
                            return
                        end
...
function FormGoFightPanel:onClickSaveBtn()
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EliteKingMatch then
        self:onFightEliteMatch(2)
...
function FormGoFightPanel:onFightPractiseTower()
    ...
    if self.setting and self.setting.power and self.cur_power and self.setting.power> self.cur_power then
        ...
        local str = string.format(TI18N("语言_c_3160"), self.cur_power,self.setting.power)
...
function FormGoFightPanel:onFightCrossarena(team_data_list, fun_form_type, is_drama )
    ...
    if team_data_list and next(team_data_list) ~= nil then
        ...
        if _type == 2 and CrossarenaController:getInstance():getModel():checkIsCanHideTwoTeam() and hide_num ~= 2 then
            ...
            if hide_cfg then
                message(string_format(TI18N("语言_c_4331"), hide_cfg.val))
            end
            return
        end
...
--通关条件刷新
function FormGoFightPanel:setPosTipColor()
    if self.condition then
        local status = 1
        local targetNum = self.condition[1][3]
        if #self.five_hero_vo ~= 0 then 
            local num = 0
            for i, v in pairs(self.five_hero_vo) do
                for j, k in pairs(v) do
                    if self.condition[1][1][1] == "combat_result" then --战斗胜利 1 胜利
                    elseif self.condition[1][1][1] == "combat_camp_type" then --阵营 水火风阳阴 camp_type
                        if k.camp_type == self.condition[1][1][2] then
                            num = num + 1
                        end
                    elseif self.condition[1][1][1] == "career_type" then --职业 远程 近程 防御 辅助 type
                        if k.type == self.condition[1][1][2] then
                            num = num + 1
                        end
                    elseif self.condition[1][1] == "formation_type" then --阵位 
                        local cfg = Config.FormationData.data_form_data[self.formation_type]
                        if cfg.order == self.condition[1][3] then
                            status = 2
                        else
                            status = 1
                        end
                    elseif self.condition[1][1][1] == "combat_order" then -- 战斗时间
                    elseif self.condition[1][1][1] == "dead_num" then --阵亡数量
                    elseif self.condition[1][1][1] == "attk_hp_per" then --存活忍者血量
                    end
                end
            end
            local condition = self.condition[1][2]
            if condition == "==" then
        ...
        if status == 2 then
            self.pos_tips:setTextColor(cc.c3b(14,168,14))
        else
...
--@tips 关卡id
function FormGoFightPanel:openRootWnd(fun_form_type, setting, show_type, tips)
    ...
    local id = self.setting.level_id
    if id then
        local tips = Config.DungeonHeavenData.data_star_cond[id].type
        self.condition = Config.DungeonHeavenData.data_star_cond[id].condition
        self.pos_tips:setString(tips)
        self.pos_tips:setTextColor(cc.c3b(201,38,6))
    end
    --先初始化按钮
    self:initTabBtnList()
...
function FormGoFightPanel:initFormSetInfo()
    ...
    -- elseif self.fun_form_type == PartnerConst.Fun_Form.HeavenBoss then
        -- self.pos_tips:setString(TI18N("语言_c_3163"))
        -- self.pos_tips:setTextColor(cc.c3b(201,38,6))
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.LimitExercise then
        -- self.pos_tips:setString(TI18N("语言_c_3164"))
    elseif self.fun_form_type == PartnerConst.Fun_Form.Adventure_Mine_Def then
        --秘矿冒险 防守
        if self.setting.is_occupy and self.save_btn then
            self.save_btn:getChildByName("label"):setString(TI18N("语言_c_118"))
        end
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EndLessWater 
        ...
        if Endless_trailEvent.type_name[self.fun_form_type] then
            self.pos_tips:setString(string_format(TI18N("语言_c_3165"),Endless_trailEvent.type_name[self.fun_form_type]))
...
function FormGoFightPanel:initTabBtnList()
    ...
    if self.fun_form_type == PartnerConst.Fun_Form.Drama then
        if self.form_show_type == HeroConst.FormShowType.eFormFight then --出战 
            self:setTabData(self.fun_form_type, TI18N("语言_c_464"), 1)
        else
            --剧情保存布阵 要显示好多
    ...
    elseif self.fun_form_type == PartnerConst.Fun_Form.EliteMatch then
        ...
        self:setTabData(self.fun_form_type, TI18N("语言_c_3152"), 1, 1,is_multiple)
        self:setTabData(self.fun_form_type, TI18N("语言_c_3153"), 2, 2,is_multiple)
    elseif self.fun_form_type == PartnerConst.Fun_Form.HeavenBoss then
        self:setTabData(self.fun_form_type, TI18N("语言_c_3152"), 1, 1,true)
        self:setTabData(self.fun_form_type, TI18N("语言_c_3153"), 2, 2,true)
    elseif self.fun_form_type == PartnerConst.Fun_Form.CrossArena or 
        self.fun_form_type == PartnerConst.Fun_Form.CrossArenaDef or 
        self.fun_form_type == PartnerConst.Fun_Form.ArenapeakchampionDef then

        self:setTabData(self.fun_form_type, TI18N("语言_c_479"), 1, 0,false)
        self:setTabData(self.fun_form_type, TI18N("语言_c_3152"), 2, 1,true)
        self:setTabData(self.fun_form_type, TI18N("语言_c_3153"), 3, 2,true)
        self:setTabData(self.fun_form_type, TI18N("语言_c_3154"), 4, 3,true)
    else
        self:setTabData(self.fun_form_type, TI18N("语言_c_464"), 1)
    end
...
function FormGoFightPanel:setTabData(fun_form_type, name, index, order, is_multiple, is_check)
    ...
    if is_check then
        ...
        if fun_form_type == PartnerConst.Fun_Form.Arena or fun_form_type == PartnerConst.Fun_Form.ArenaChampion  then 
            ...
            tab_data.is_lock_des = desc or TI18N("语言_c_1586")
...
function FormGoFightPanel:createNewCellTabBtn(width, height)
    ...
    cell.label:getVirtualRenderer():setLineSpacing(-12)
    --红点.目前处以隐藏..还没有需呀用到
...
function FormGoFightPanel:changeTabType(index, check_repeat_click, is_move_tab)
        ...
        if index == 2 and tab_data.is_lock then
            if self.setting.match_type == ElitematchConst.MatchType.eKingMatch then
                ...
                if config then
                    ...
                    message(TI18N("语言_c_3166"))
                end
            else
                message(TI18N("语言_c_3167"))
            end
            return 
        end
    else
    ...
    if self.cur_tab ~= nil and not self.tab_list[self.cur_tab.index].is_lock then
...
function FormGoFightPanel:updateHallowsIcon()
    ...
    if self.hallows_id == 0 then
        ...
        self.equip_btn_label:setString(TI18N("语言_c_3168"))
    end
end
...
function FormGoFightPanel:calculateCampHaloType()
    ...
    local halo_icon_config, alien_effect = BattleController:getInstance():getModel():getCampIconConfigByIds(self.halo_form_id_list)
...
--播放特效
function FormGoFightPanel:updateEffect(status, halo_res, alien_effect)
    if status then
...
function FormGoFightPanel:updateCellByIndex(cell, index)
    ...
    if self.fun_form_type == PartnerConst.Fun_Form.Adventure_Mine_Def then  --秘矿冒险
        if self.dic_mine_hero_list and self.dic_mine_hero_list[hero_vo.id] then
            cell:showStrTips(true,TI18N("语言_c_3171"),{c3b = cc.c3b(255,255,255)})
        else
...
function FormGoFightPanel:selectHero(item, hero_vo)
    ...
    if self.fun_form_type == PartnerConst.Fun_Form.Expedit_Fight then
        ...
        if is_used == 1 then message(TI18N("语言_c_3173")) return end
...
function FormGoFightPanel:setTimeFormatString(time)
    if time <= 0 then
        ...
        if self.fun_form_type == PartnerConst.Fun_Form.Adventure_Mine_Def then --矿战
            self.down_time:setString(string_format(TI18N("语言_c_3176"), time))
        elseif self.fun_form_type == PartnerConst.Fun_Form.EliteMatch then --段位赛
            self.down_time:setString(string_format(TI18N("语言_c_3177"), time))
        end
    end
end
