...
function ArtifactRecastWindow:open_callback(  )
	...
	main_container:getChildByName("win_title"):setString(TI18N("语言_c_2982"))
    ...
    self.check_panel:getChildByName("box_lab"):setString(TI18N("语言_c_2983"))
...
function ArtifactRecastWindow:adjustNodePos(  )
    ...
    if self.item_config.quality >= BackPackConst.quality.orange then
        if not self.tips_text then
            self.tips_text = createRichLabel(22, nil, cc.p(0.5, 0.5), cc.p(340, 210), nil, nil, 660)
            self.main_container:addChild(self.tips_text)

            local function clickLinkCallBack( _type, value )
                if _type == "href" then
                    if self.item_config.id == BackPackConst.Artifact.artifact_id_3 then --星耀符文
                        self.ctrl:openArtifactSkillWindow(true, 1, 4)
                    else
        ...
        self.progress_bg:setPositionY(138)
    ...
    if self.item_config and self.item_config.quality >= BackPackConst.quality.orange then
        ...
        if self.tips_text then
            ...
            if less_num <= 0 and cur_count > 0 then
                self.reset_btn_label:setString(TI18N("语言_c_2984"))
                self.cancel_btn_label:setString(TI18N("语言_c_2984"))
                self.tips_text:setString(TI18N("语言_c_2985"))
                ...
                self.reset_btn_label:setString(TI18N("语言_c_763"))
                self.cancel_btn_label:setString(TI18N("语言_c_763"))
                local str = string_format(TI18N("语言_c_2986"), less_num)
                if self.checkbox:isSelected() == true then
                    if self.select_luck_type and self.select_luck_type == 2 then
                        str = string_format(TI18N("语言_c_2987"), less_num)
                    else
                        str = string_format(TI18N("语言_c_2988"), less_num)
                    end
...
function ArtifactRecastWindow:setBaseAttrInfo(  )
    ...
    local temp_posx = 414
    if #self.data.attr == 2 then
        temp_posx = 396.5
    elseif #self.data.attr == 1 then
        temp_posx = 379
    end
    for i,v in ipairs(self.data.attr) do
        ...
        if attr_name then
            if not self.base_list_left[i] then 
                self.base_list_left[i] = createRichLabel(22, cc.c4b(104,69,42,255), cc.p(0, 0.5), cc.p(20, 28), nil, nil, 380)
                self.left_bg:addChild(self.base_list_left[i])
            end
            local label = self.base_list_left[i]
            local _x = 30
            local _y = temp_posx - (i-1)*35
        ...
        for i,v in ipairs(self.data.extra_attr) do
            ...
            if attr_name then
                if not self.base_list_right[i] then 
                    self.base_list_right[i] = createRichLabel(22, cc.c4b(104,69,42,255), cc.p(0, 0.5), cc.p(20, 28), nil, nil, 380)
...
function ArtifactRecastWindow:createSkillItem(parent, index)
    ...
    local pos_y = 225 - (index-1)*100
    skill:setPosition(60, pos_y)
    local name = createLabel(20,cc.c4b(104,69,42,255),nil,110, pos_y+25,"",parent,1,cc.p(0,0))
    ...
    local random_des = createLabel(20,cc.c4b(104,69,42,255),nil,110, pos_y,TI18N("语言_c_2944"),parent,1,cc.p(0,0.5))
    item.skill = skill
    item.name = name
    item.desc = desc
