...
function ArtifactTipsWindow:open_callback()
    ...
    self.baseattr_lab = self.baseattr_panel:getChildByName("label")
    self.baseattr_lab:setString(TI18N("语言_c_1282"))
    ...
    self.skill_panel_1:getChildByName("label"):setString(TI18N("语言_c_2940"))
    self.skill_pre_btn = self.skill_panel_1:getChildByName("skill_pre_btn")
    self.skill_pre_btn:getChildByName("label"):setString(TI18N("语言_c_2287"))
    ...
    local title_list ={[1]=TI18N("语言_c_976"),[2]=TI18N("语言_c_763"),[3]=TI18N("语言_c_2958"),[4]=TI18N("语言_c_2959"),}
    for i=1,4 do 
...
function ArtifactTipsWindow:register_event()
    ...
    registerButtonEventListener(self.skill_pre_btn, function (  )
        if self.data and self.data.config.id == BackPackConst.Artifact.artifact_id_3 or BackPackConst.Artifact.artifact_id_4 then
			HeroController:getInstance():openArtifactSkillWindow(true, nil, 4)
		elseif self.data and self.data.config.id == BackPackConst.Artifact.artifact_id_2 then
			HeroController:getInstance():openArtifactSkillWindow(true, nil, 3)
		else
...
function ArtifactTipsWindow:clickBtn(index)
    ...
    elseif index == PartnerConst.Artifact_Type.Getoff then
        if self.is_off_tips then
            ...
            end, TI18N("语言_c_62"),nil,nil,nil,{title = TI18N("语言_c_2961")})
        else
            if self.data and self.data.id then
                controller:sender11030(self.partner_id,self.artifact_pos,self.data.id,0)
            end
    ...
    elseif index == PartnerConst.Artifact_Type.Resolve then
        if self.data and self.data.config and self.data.config.id and self.data.enchant then
            ...
            if config and config[1] then
                ...
                local msg = string.format(TI18N("语言_c_2962"),PathTool.getItemRes(bid),num)
                CommonAlert.show(
                    msg,
                    TI18N("语言_c_63"),
                    function()
...
function ArtifactTipsWindow:openRootWnd(data, open_type, partner_id, pos, is_off_tips, setting)
    ...
    local y = self.baseattr_panel:getContentSize().height
    if self.data.attr and math.floor((#self.data.attr - 1) / 2) * 35 and y > 0 then
        target_height = target_height + y
		self.baseattr_panel_height = 90 + y
		self.baseattr_panel:setContentSize(cc.size(422, self.baseattr_panel_height))
    end
    -- 底部按钮
    if self.open_type == PartnerConst.ArtifactTips.partner then
        --target_height = target_height - 50
        self.tab_list[3]:setTitleText(TI18N("语言_c_2964")) 
        self.tab_list[3].index = PartnerConst.Artifact_Type.Getoff 
        self.tab_list[4]:setTitleText(TI18N("语言_c_2965"))
    ...
    elseif self.open_type == PartnerConst.ArtifactTips.sell then
        self.tab_list[1]:setTitleText(TI18N("语言_c_1711")) 
...
function ArtifactTipsWindow:initLimitBuyInfo()
    ...
    if self.data.limit_day then --每日限购
        ...
        if label then
            label:setString(string_format(TI18N("语言_c_1713"), self.data.limit_day, total_count))
        end
    end
end
...
function ArtifactTipsWindow:addMarketplaceTime()
    if self.setting and self.setting.is_market_place and self.data.end_time and next(self.data.end_time) ~= nil then
        ...
        if time <= 0 then
            str = string.format(TI18N("语言_c_2966"))
        else
            str = string.format(TI18N("语言_c_2967"), TimeTool.GetTimeFormatDayIIIIII(time))    
        end 
...
function ArtifactTipsWindow:setBaseAttrInfo()
    ...
    local temp_height = self.baseattr_panel_height
    temp_height = temp_height - 24
    self.baseattr_lab:setPositionY(temp_height)
    temp_height = temp_height - 38
    ...
    for i,v in ipairs(self.data.attr) do
        ...
        if attr_name then
            if not self.base_list[index] then 
                self.base_list[index] = createRichLabel(22, cc.c4b(0xc1,0xb7,0xab,0xff), cc.p(0, 0.5), cc.p(20, temp_height), nil, nil, 380)
                self.baseattr_panel:addChild(self.base_list[index])
            end
            local label = self.base_list[index]
            local _x = 30 + (i-1)%2 * 200
            
            local _y = temp_height - math.floor((i-1)/2)*35
    ...
    --[[if (index-1) < 2 then
        if artifact_config and artifact_config[cur_star] then
            if artifact_config[cur_star].next_attr_star > 0 then
                ...
                label:setString(string_format(TI18N("语言_c_2968"), artifact_config[cur_star].next_attr_star, StringUtil.numToChinese(index)))
            end
        end
    end--]]
end
    ...
    --[[if index <= 3 then
        ...
        if artifact_config and artifact_config[cur_star] then
            if artifact_config[cur_star].next_skill_star > 0 then
                ...
                skill_item.desc:setString(string_format(TI18N("语言_c_2969"), StringUtil.numToChinese(artifact_config[cur_star].next_skill_star), index))
