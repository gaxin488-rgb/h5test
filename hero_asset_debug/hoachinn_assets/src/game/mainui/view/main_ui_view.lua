...
function MainUiView:__init()
    self.res_list = {
        ...
        {path = PathTool.getPlistImgForDownLoad("board", "board"), type = ResourcesType.plist},
...
function MainUiView:createRootWnd()
    ...
    self.mainui_1 = self.bottom_container:getChildByName("mainui_1")
    -- mainui_1 = MainUiView.mainui_1
    self.mainui_2 = self.bottom_container:getChildByName("mainui_2")
    for i = 1, 7 do
        local btn = self.mainui_1:getChildByName("mainui_tab_"..i)
    ...
    for i = 1, 4 do
        local btn = self.mainui_2:getChildByName("heaven_chapter_btn_"..i)
        registerButtonEventListener(btn,function()
            self:heavenCopyJump(i)
        end,true)
        -- btn:addTouchEventListener(function()
        --     self:heavenCopyJump(i)
        -- end)
    end
    self.dialBtn = self.mainui_2:getChildByName("heaven_chapter_btn_1")
    self.tips_point = self.dialBtn:getChildByName("tips_point")
    self:setDialRed()
    -- self:changeBottomShowStatus(1)
    self.top_container = self.root_wnd:getChildByName("top_container")
    self.exp_bar = self.bottom_container:getChildByName("exp_bar")
    ...
    self.fight_value:setString(role_vo.power)
    -- self.fight_bg:setContentSize(cc.size(self.fight_txt:getContentSize().width + self.fight_value:getContentSize().width + 30))
    self.fight_value:setPositionX(self.fight_txt:getContentSize().width + self.fight_txt:getPositionX()+10)
end

function MainUiView:showServerTime()
	...
	--print("===============server_time==",Servertime)
        ...
        self.server_time:setString(TimeTool.getHM(Servertime))
	--print("===============server_time==",TimeTool.getHM(Servertime))
    end))))
end

function MainUiView:setDialRed()
    local model = HeavenController:getInstance():getModel() 
    local bool1 = model:getHeavenDialIsFreeById(1)
    local bool2 = model:getHeavenDialIsFreeById(2)
    local bool = false
    if bool1 or bool2 then
        bool = true        
    end
    self.tips_point:setVisible(bool)
end
...
function MainUiView:setMaiuiBtnStatus(index)
    if self.cur_select_btn ~= nil then
        self.cur_select_btn.btn:stopAllActions()
    ...
    if self.cur_select_btn ~= nil then
        if self.cur_select_index == MainuiConst.btn_index.drama_scene then
            ...
            self.cur_select_btn.selected:setVisible(true)
            self.cur_select_btn.btn:setPositionY(self.cur_select_btn.init_y)
            self.cur_select_btn.btn:runAction(cc.Sequence:create(cc.DelayTime:create(0.1), cc.CallFunc:create(function()
...
--@return 
--==============================--
function MainUiView:heavenCopyJump(index)
   if index == MainuiConst.heavenCopy_type.pray then
        HeavenController:getInstance():openHeavenDialWindow(true,1)
        -- local page = HeavenController:getInstance():openHeavenDialWindow(true,1)
   elseif index == MainuiConst.heavenCopy_type.books then
        HeroController:getInstance():openHeroClothesLustratWindow(true)
   elseif index == MainuiConst.heavenCopy_type.ranking then
        HeavenController:getInstance():openHeavenRankWindow(true)
   elseif index == MainuiConst.heavenCopy_type.shop then
...
--@return 
--==============================--
function MainUiView:changeBottomShowStatus(index)
    if index == 1 then
        self.mainui_1:setPositionY(0)
        self.mainui_2:setPositionY(-200)
    else
        if HeavenController:getInstance():getHeavenChapterWindowIsOpen() then
            self.mainui_1:setPositionY(-200)
            self.mainui_2:setPositionY(0)
        end
    end
end
    ...
    if is_click_status then
        if (GuideController:getInstance():isInGuide() == true) or (force == true) then -- 如果在引导中
            ...
            if not self.tips_alert then
                self.tips_alert = CommonAlert.show(str, TI18N("语言_c_63"), fun, TI18N("语言_c_62"), cancelfunc, CommonAlert.type.rich, cancelfunc, nil, nil, true)
            end
        end
    end 
    ...
    if _type == FunctionIconVo.type.right_top_2 then
        if next(self.layout_list[_type]) ~= nil then
            ...
            if not self.right_icon_bg then
                ...
                self.right_icon_bg = createImage(icon_container, PathTool.getResFrame("common", "mainui_1036"), con_size.width*0.5, 175, cc.p(0.5, 1), true, -1, true)
...
function MainUiView:setIconPosition(icon, index, layout, type)
    ...
    elseif type == FunctionIconVo.type.right_top_2 then
        _x = size.width * 0.5 
        _y = size.height - icon.height * 0.5 - (index - 1) * (icon.height + off_height + 10)
...
function MainUiView:changeChallengeEffectStatus(status)
    ...
    if status == true then
        ...
        self.challenge_effect:removeFromParent()
        self.challenge_effect = nil
        self.challenge_effect = createEffectSpine(PathTool.getEffectRes(310), cc.p(5,10), cc.p(0, 0), true, PlayerAction.action_1)
        self.bottom_btn_list[MainuiConst.btn_index.drama_scene].icon:addChild(self.challenge_effect)
...
function MainUiView:registerEvent()
    ...
    self.effect_container:addTouchEventListener(function(sender, event_type)
        if event_type == ccui.TouchEventType.ended then
            ...
            if self.training_camp_lev_limit and self.training_camp_lev_limit.val > role_vo.lev then
                message(string_format(TI18N("语言_c_1004"),self.training_camp_lev_limit.val))
                return
            end
    ...
    if self.fight_panel then
        self.fight_panel:addTouchEventListener(function()
            self.fight_value:setString(role_vo.power)
...
function MainUiView:updataExchangeRedPoint()
    ...
    for i,v in ipairs(self.exchange_data.list) do
        if v.id == 1 and self.coin_redpoint then
            ...
            addRedPointToNodeByStatus(self.coin_redpoint,status,11,7,nil,nil,PathTool.getResFrame("mainui","mainui_1009"))
        end
    end
end
...
function MainUiView:createChatBubble()
    ...
    self.chat_bubble:setPosition(self.handle_x, self.handle_y + 380)
...
function MainUiView:_onClickPromptTips( status )
    if status == false then
        ...
        local temp_pos = cc.p(50.38+59,68.23)
...
--获取道具移动相应的位置
function MainUiView:getItemFlyEndPos(bid)
    local pos = cc.p(self.size.width/2, self.size.height)
    local node = nil
    if bid == Config.ItemData.data_assets_label2id.coin then
        node = self.info_container:getChildByName("wealth_icon_1")
    elseif bid == Config.ItemData.data_assets_label2id.exp then
        node = self.top_container:getChildByName("head_container")
    elseif bid == Config.ItemData.data_assets_label2id.gold then
        node = self.info_container:getChildByName("wealth_icon_2")
    else
        node = self.mainui_1:getChildByName("mainui_tab_3")
    end
    local size = node:getContentSize()
    local w_pos = node:convertToWorldSpace(cc.p(size.width/2, size.height/2))
    pos = self.item_container:convertToNodeSpace(w_pos)
    return pos
end

function MainUiView:shakeRelevantImage(bid)
    local node = nil
    if bid == Config.ItemData.data_assets_label2id.coin then
        node = self.info_container:getChildByName("wealth_icon_1")
    elseif bid == Config.ItemData.data_assets_label2id.exp then
    ...
    if node.isShaking then
        return
    end
    local scale = node:getScale()
    local scaleTo1 = cc.ScaleTo:create(0.1, scale*1.1)
    local scaleTo2 = cc.ScaleTo:create(0.1, scale)
    local callfunc = cc.CallFunc:create(function ()
        node.isShaking = false
    end)
    node.isShaking = true
    node:runAction(cc.Sequence:create(scaleTo1, scaleTo2, callfunc))
end
