...
function EndlessTrailPanel:configUI(...)
    ...
    rank_desc_label:setString(TI18N("语言_c_1096"))
    ...
    label:setString(TI18N("语言_c_109"))
    label:enableOutline(Config.ColorData.data_color4[264],2)
    self.first_label = createRichLabel(20, cc.c4b(0x64,0x32,0x23,0xff), cc.p(1, 1), cc.p(325, self.first_container:getContentSize().height-8),nil,nil,360)
    self.first_container:addChild(self.first_label)
    self.limit_label = createRichLabel(20, cc.c4b(0x95,0x53,0x22,0xff), cc.p(1, 0.5), cc.p(self.first_container:getContentSize().width-20, 43),nil,nil,250)
    ...
    self.info_lab = createRichLabel(20, cc.c4b(0x24,0x90,0x03,0xff), cc.p(1, 0.5), cc.p(self.info_touch:getContentSize().width, self.info_touch:getContentSize().height*0.5-10),10,nil,250)
    self.info_lab:setString(TI18N("语言_c_1097"))
    -- self.info_lab:setPositionX()
    ...
    self.my_title:setString(TI18N("语言_c_4808"))
    ...
    self.rank_notice:setString(TI18N("语言_c_1098"))
    ...
    round_desc_label:setString(TI18N("语言_c_1099"))
    ...
    self.friend_container:getChildByName("label_1"):setString(TI18N("语言_c_1100"))
    ...
    self.from_label = createRichLabel(22, cc.c4b(0x95, 0x53, 0x22, 0xff), cc.p(1, 0.5), cc.p(240,56))
    ...
    self.can_reward_label:setString(TI18N("语言_c_1101"))
	...
	self.bottom_panel:setPositionY(bottom_off + 266)--180)
end
...
function EndlessTrailPanel:register_event( ... )
    ...
    registerButtonEventListener(self.tips_button, function()
        if  Config.EndlessData.data_explain and Config.EndlessData.data_explain[1] then
            MainuiController:getInstance():openCommonExplainView(true,  Config.EndlessData.data_explain[1], TI18N("语言_c_76"))
        end
    end,true, 1)
...
function EndlessTrailPanel:updateBaseInfo()
    ...
    if self.base_data then
        ...
        self.from_label:setString(string.format(TI18N("语言_c_1104"),data.current_round))


        self.has_label:setString(string.format(TI18N("语言_c_1105"),data.day_pass_round))
        if data.day_pass_round == 0 then
            self.ack_btn_label:setString(TI18N("语言_c_1095"))
        else
            self.ack_btn_label:setString(TI18N("语言_c_1106"))
        end
        ...
        if data.is_employ == FALSE and next(data.list or {}) == nil then --没雇佣伙伴
            self.friend_label:setString(TI18N("语言_c_1107"))
        ...
        if self.first_data.id == 0 then
            ...
            if first_data then
                if not self.item_scrollview then
                    ...
                    self.item_scrollview = CommonScrollViewLayout.new(self.first_container, cc.p(-20,5) , ScrollViewDir.horizontal, ScrollViewStartPos.bottom, cc.size(195,85), setting)
                end
                local str = string.format(TI18N("语言_c_1079"),first_data.limit_id)
                ...
                if self.first_data.status == 1 then
                    ...
                    str = string.format(TI18N("语言_c_1109"), first_data.limit_id - self.base_data.max_round)
                    self.limit_label:setString(str)
                    self.limit_label:setVisible(true)
                    self.get_btn:setVisible(false)
                end
...
function EndlessTrailPanel:updateRankItem()
    ...
    if config == nil then   -- 未上榜
        ...
        self.my_rank_value:setString(TI18N("语言_c_1110"))
    else
    ...
    self.my_title:setPositionX(self.my_rank_value:getPositionX() - self.my_rank_value:getContentSize().width - 10)
end
