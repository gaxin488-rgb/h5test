...
function EndlessTrailBattleView:configUI( ... )
   ...
   self.backpack_btn:setVisible(false)
   self.backpack_label = createRichLabel(22, 1,cc.p(0.5,1),cc.p(self.backpack_btn:getContentSize().width/2 - 4,8),-4, nil, 100)
   ...
   self.left_container:setPositionY(0)
    ...
    if self.return_btn then
        self.return_btn:addTouchEventListener(function(sender,event_type )
            if ccui.TouchEventType.ended == event_type then
                ...
                CommonAlert.show(str,TI18N("语言_c_1118"),confirm_callback,TI18N("语言_c_1119"),nil,CommonAlert.type.rich)
            end
        end)
    end
...
function EndlessTrailBattleView:updateData(data)
    ...
    if final_data then
        ...
        local str = string.format(TI18N("语言_c_1120"),final_data.round)
        self:playFightStartEffect(final_data.round)
        local str_2 = TI18N("语言_c_1121")
        if final_data.buff_list and next(final_data.buff_list or {}) ~= nil then
            ...
            str_2 = string.format(TI18N("语言_c_4732"),str_2)
        end
        self.desc_label_2:setString(str_2)
        self.desc_label:setString(str)

        local str_2 = TI18N("语言_c_44")
        if final_data.acc_reward and next(final_data.acc_reward) then
            ...
            if final_data.reward_flag == TRUE then
                str_2 = string.format(TI18N("语言_c_4733"),str_2)
            else
                str_2 = string_format(TI18N("语言_c_1122"), str_2, final_data.rest_round or 0, final_data.max_reward_round or 0)
            end
        end
...
function EndlessTrailBattleView:playFightStartEffect(round)
    ...
    if self.effect_container == nil then
        ...
        --这里会切下一波战斗，无尽切换，所以在这里加个清理，防止内存占用过多，这边还需要评估一下
        if display.removeUnusedSpine then
            display.removeUnusedSpine()
            --这里也需要考虑好下一波的预加载
        end
    end
    ...
    if self.battle_data and data and Config.EndlessData.data_first_data[self.battle_data.type] and Config.EndlessData.data_first_data[self.battle_data.type][data.id] then
        ...
        if first_data then
            ...
            if first_data.items and first_data.items[1] then
                self.backpack_btn:setVisible(true)
            ...
            if data.status == 1 then
                ...
                str = string_format(TI18N("语言_c_1123"))
                ...
                str = string_format(TI18N("语言_c_1124"), first_data.limit_id) -- - self.battle_data.max_round
