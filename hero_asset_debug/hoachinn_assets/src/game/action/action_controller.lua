...
function ActionController:handle13601(data)
	...
	if data then
		if data.charge then
			data.charge = getMoneyLangNum(data.charge)
		end
		local list = data.welfare_list
		if list then 
			for k,v in pairs(list) do
				v.target_val = getMoneyLangNum(v.target_val)
				v.value = getMoneyLangNum(v.value)
			end
		end
    end
...
function ActionController:checkOpenActionLimitGiftMainWindow(gift_id)
	...
	if config then
		...
		CommonAlert.show(config.desc,TI18N("语言_c_108"),comfirm_callback, TI18N("语言_c_62"), cancel_callback)
	else
		GlobalEvent:getInstance():Fire(BattleEvent.NEXT_SHOW_RESULT_VIEW)
	end
end
	...
	if remain_count >= max_count and cur_count == 0 then
		...
		if cur_count <= 0 then
			...
			if buy_config and next(buy_config.expend) ~= nil then
	            ...
	            local str = string.format(TI18N("语言_c_634"), iconsrc, count)
...
function ActionController:openOuqiGiftTips()
    ...
    local str = TI18N("语言_c_4362")
    CommonAlert.show(str, TI18N("语言_c_63"), call_back, nil, nil, CommonAlert.type.rich, nil, nil, nil, true)
end
...
-- 取是否跳过动画
--支持发单个或多个id
function ActionController:sender16808(types)
	if not types or MAKELIFEBETTER_NEW then return end
	...
	if next(types) ~= nil then
		...
		for index, value in ipairs(types) do
			table.insert(type_list,{type=value})
		end
		local protocal = {}
		protocal.type_list = type_list
		self:SendProtocal(16808, protocal)
	end
end
function ActionController:handle16808(data)
	if data and data.type_info_list then
		local type_info_list = data.type_info_list
		for key, value in pairs(type_info_list) do
			self:setDataHandle16808(value)
		end
	end
end
function ActionController:setDataHandle16808(data)
	local FullReduction_campid = 0
	if self:getActionSubTabVo(ActionRankCommonType.FullReduction) and self:getActionSubTabVo(ActionRankCommonType.FullReduction).camp_id then
		FullReduction_campid = self:getActionSubTabVo(ActionRankCommonType.FullReduction).camp_id	
	end
	local optional_gift2_campid = 0
	if self:getActionSubTabVo(ActionRankCommonType.optional_gift2) and self:getActionSubTabVo(ActionRankCommonType.optional_gift2).camp_id then
		optional_gift2_campid = self:getActionSubTabVo(ActionRankCommonType.optional_gift2).camp_id	
	end
	if data.type == ActionStorageBool.IsJumpOverAnimation then
		NewPartnersummonController:getInstance():getModel():setignoreMp4(data.is_save)
	elseif data.type == ActionStorageBool.PoolParty then
		MonopolyController:getInstance():getModel():setIsSkipAnimation(data.is_save)
	elseif data.type == ActionStorageBool.AdventureMopup then
		local uimodel = AdventureController:getInstance():getUiModel()
		if uimodel and uimodel.setMopupStartStatus then
			uimodel:setMopupStartStatus(data.is_save)
		end
	elseif data.type == ActionStorageBool.FullReduction + FullReduction_campid then
		self:setHolidayStatus(ActionRankCommonType.FullReduction,(data.is_save == 0))
	elseif data.type == ActionStorageBool.optional_gift2 + optional_gift2_campid then
		self:setHolidayStatus(ActionRankCommonType.optional_gift2,(data.is_save == 0))
	elseif data.type == ActionStorageBool.MoreMonopolyBoss then
		MonopolyController:getInstance():getModel():setIsTip(data.is_save)
	elseif data.type == ActionStorageBool.BonsSkill then
		HeroController:getInstance():getModel():setIsFirst(data.is_save)
	elseif data.type == ActionStorageBool.Form_BonsSkill then
		HeroController:getInstance():getModel():setFormBondTakeEffect(data.is_save)
	elseif data.type == ActionStorageBool.Heaven_Hide then
		HeavenController:getInstance():getModel():setIsLookHeavenHide(data.is_save == 1)
	elseif data.type == ActionStorageBool.ResonateHigh then
		HeroController:getInstance():getModel():setIsLookSpeed(data.is_save == 0)
	elseif data.type == 777 then
		VoyageController:getInstance():getModel():setOneKeyGetStatus(data.is_save)
	elseif data.type == 778 then
		VoyageController:getInstance():getModel():setOneKeyAcceptStatus(data.is_save)
	elseif data.type == ActionRankCommonType.Turkey then
		local tab_vo = self:getActionSubTabVo(data.type)
		if tab_vo then
			self:openActionTurkeyShowPanel(data.is_save == 0,tab_vo.camp_id,data.type)
		end
	elseif data.type == ActionStorageBool.TreasureResearchFinish then
		TreasureController:getInstance():getModel():setFirstFinish(data.is_save == 0)
	elseif data.type == ActionStorageBool.TreasureResearchInto then
		TreasureController:getInstance():getModel():setFirstInto(data.is_save == 0)
	elseif data.type == 60212+100000 or data.type == 89910+100000 or data.type == 89911+100000 then
		BackpackController:getInstance():getModel():getFirstChargeRedPoint(data)
	elseif data.type == ActionStorageBool.BreakNew then
		TaskController:getInstance():getModel():setBreakNewRed(data)
	elseif data.type == ActionStorageBool.ForgeHouse_Gold_Equip then
		ForgeHouseController:getInstance():getModel():setForgeHouseGoldEquipRed(data.is_save == 0)
	elseif data.type == ActionStorageBool.SpSummon then
		NewPartnersummonController:getInstance():getModel():setFirstOpenSpSummon(data.is_save == 0)
	elseif data.type == ActionStorageBool.SpElitePanel then
		ActionController:getInstance():getModel():setfirstSpElfin(data.is_save == 0)
	elseif data.type == ActionStorageBool.NewGrowFund then
		ActionController:getInstance():getModel():setNewGrowFundFirst(data.is_save == 0)
	elseif data.type == ActionStorageBool.returnResBack then
		NewReturnActionController:getInstance():getModel():setReturnResBackFirst(data.is_save == 0)
	elseif data.type == ActionStorageBool.gemAutoRemove then
		GemstoneController:getInstance():getModel():setHeroSacrificeNum(data.is_save)
	elseif data.type == ActionStorageBool.gemIsAutoRemove then
		GemstoneController:getInstance():getModel():setGemIsSacrifice(data.is_save == 1)
	elseif data.type == ActionStorageBool.gemIsSkipSmelt then
		GemstoneController:getInstance():getModel():setSkipSmelt(data.is_save)
	elseif data.type == ActionStorageBool.forgeIsSkipSmelt then
		ForgeHouseController:getInstance():getModel():setSkipSmelt(data.is_save)
	elseif data.type == ActionStorageBool.gemBackRed then
		GemstoneController:getInstance():getModel():setGemStoneBackRed(data.is_save == 0)
	elseif data.type == ActionStorageBool.gemStoneRed then
		GemstoneController:getInstance():getModel():setGemStoneRed(data.is_save == 0)
	elseif data.type == ActionStorageBool.btIsSkipSmelt then
		ActionController:getInstance():getModel():setBTSkipSpine(data.is_save)
	elseif data.type == ActionStorageBool.allBtIsSkipSmelt then
		BossTreasureController:getInstance():getModel():setBTSkipSpine(data.is_save)
	elseif data.type == ActionStorageBool.allBtHolidayFirstRed then
		BossTreasureController:getInstance():getModel():setBTHolidayFirstRed(data)
	elseif data.type == ActionStorageBool.allBtFirstRed then
		BossTreasureController:getInstance():getModel():setBTFirstRed(data)
	elseif data.type == ActionStorageBool.URSkillClick then
		HeroController:getInstance():getModel():setUrSkillClick(data)
	elseif data.type == ActionStorageBool.UrElitePanel then
		ActionController:getInstance():getModel():setfirstUrElfin(data)
	elseif data.type == ActionStorageBool.xiaoxiaoleFirst then
		MooncombatController:getInstance():getModel():setfirstXXL(data)
	elseif data.type == ActionStorageBool.olympicGameTips then
		MooncombatController:getInstance():getModel():setfirstOlympicGameTips(data)
	elseif data.type == ActionStorageBool.mooncombat_directpurchase then
		MooncombatController:getInstance():getModel():updatefirstDirectpurchase(data)
	elseif data.type == ActionStorageBool.NeworderactionPop then
		NeworderactionController:getInstance():getModel():setNeworderactionPop(data)
	elseif data.type == ActionStorageBool.autoRemoveElfin then
		ElfinController:getInstance():getModel():setAutoRemove(data.is_save)
	elseif data.type == ActionStorageBool.puzzleBook then
		HomeworldController:getInstance():getModel():setPuzzleFirstRed(data)
	elseif data.type == ActionStorageBool.ninjaVillageCombat then
		NinjaVillageCombatController:getInstance():getModel():setSeasonFirstData(data)
	elseif data.type == ActionStorageBool.angerUpLevCostType then
		HeroController:getInstance():getModel():setAngerUpLevCostType(data.is_save)
	elseif data.type == ActionStorageBool.bondSkillCostType then
		HeroController:getInstance():getModel():setBondSkillCostType(data.is_save)
	elseif data.type > 2000 and data.type <= 2999 then
		TreasureController:getInstance():getModel():setResearchFirstRed(data.type,data.is_save == 0)
	elseif data.type > 3000 and data.type <= 3999 then
		PotentialController:getInstance():getModel():setBtnFirstRed(data.type-2000,data.is_save == 0)
	elseif data.type > 4000 and data.type <= 4999 then-- 用作记录拼图图鉴的图鉴首次点击
		HomeworldController:getInstance():getModel():setPuzzleFirstRed(data.type-4000,data.is_save == 0)
	elseif data.type > 10000 and data.type <= 99999 then
		HeroController:getInstance():getModel():setNewConvertFirstRed(data.type,data.is_save == 0)
	elseif data.type > 100000 and data.type <= 999999 then --忍者bid刻印的首次点击事件(筛选按钮专用)
		HeroController:getInstance():getModel():setKeyinFirstRed(data.type,data.is_save == 0)
	end
end
