...
function BattlDramaQuickBattleWindow:open_callback()
    ...
    self.main_container:getChildByName("title_label"):setString(TI18N("快速战斗"))
    ...
    self.quick_btn = createButton(self.main_container, TI18N("快速作战"), self.main_container:getContentSize().width/2,426, cc.size(220, 64), PathTool.getResFrame("common", "common_1017"), 20, Config.ColorData.data_color4[1])
    ...
    self.btn_buy_quick:setPosition(cc.p(500, 720))
    local Text_2 = self.btn_buy_quick:getChildByName("Text_2")
    Text_2:setString(TI18N("前往激活"))
    self.btn_buy_quick:setVisible(false)
    local Image_2 = self.btn_buy_quick:getChildByName("Image_2")
    Image_2:setContentSize(cc.size(Text_2:getContentSize().width+5,Image_2:getContentSize().height))
...
function BattlDramaQuickBattleWindow:checkQuickBattle()
    if self.quick_data then
        ...
        local str = string.format("%s<div fontColor=#249003>%s</div>",TI18N("今日剩余:"),combat_num)

        if self.privilege_status then
            str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("今日剩余:"),combat_num, privilege_num)
        end
        ...
        if self.quick_data.is_holiday == 1 or self.privilege_status == true then
            if self.quick_data.is_holiday == 1 and self.privilege_status == true then --活动与特权同时存在
                ...
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("今日剩余:"),combat_num, holid_num+privilege_num)
            elseif self.privilege_status == true then --仅有特权
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("今日剩余:"),combat_num, privilege_num)
            else
                local holid_num = Config.DungeonData.data_drama_const.hd_fast_combat_buy_time.val - self.quick_data.fast_combat_w_num
                str = string.format("%s<div fontColor=#249003>%s+%s</div>",TI18N("今日剩余:"),combat_num, holid_num)
            end
        end
        ...
        if self.quick_data.fast_combat_free_num > 0 then --代表是免费的
            ...
            if self.privilege_status then -- 特权开启
                ...
                elseif self.quick_data.is_holiday == 1 and self.quick_data.fast_combat_w_num < const.hd_fast_combat_buy_time.val then 
                        self.quick_battle_status = 2
        ...
        self.quick_btn:setRichText(btn_str, 20)
    end
end
