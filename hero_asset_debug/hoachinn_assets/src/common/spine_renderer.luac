...
function SpineRenderer:__init(screen_pos, spine_name, scale, is_async, anima_name, career, group, spine_type, is_boss, unit_is_boss, spine_scale2)
	...
	self.hp_root_img = ccui.Widget:create()
	self.hp_root:addChild(self.hp_root_img)
...
function SpineRenderer:setName(name)
	self._name = name or ""
end

function SpineRenderer:getName()
	return self._name
end
...
function SpineRenderer:createMergeSprite(sprite)
	local spsize = sprite:getContentSize()
    local maskRt = CCRenderTexture:create(spsize.width, spsize.height)
    maskRt:setPosition(0, 0)
    maskRt:begin()
    sprite:visit()
    maskRt:endToLua()
	return maskRt
...
function SpineRenderer:playPassiveSkillName(msg, callback)
	...
	local sp_item = createSprite(PathTool.getResFrame("battle", "battle_buff_name_bg"), 0, 0, nil, cc.p(0, 0), LOADTEXT_TYPE_PLIST, 99) 
	local tips = createLabel(20, 217, 218, sp_item:getContentSize().width / 2, sp_item:getContentSize().height / 2, msg, sp_item, nil, cc.p(0.5, 0.5))
	local mergesp = self:createMergeSprite(sp_item)
	local skill_item = cc.Sprite:create()
	skill_item:initWithTexture(mergesp:getSprite():getTexture());
	skill_item:setFlipY(true);
	skill_item:setScale(0.6)
	skill_item:setOpacity(100)
	skill_item:setAnchorPoint(cc.p(0.5,0.5))
	skill_item:setVisible(false)
	self.root:addChild(skill_item,99)
	...
	skill_item:runAction(cc.Sequence:create(delay, cc.CallFunc:create(function()
			if not tolua.isnull(skill_item) then
...
function SpineRenderer:createHallowsStage()
    ...
    self.hallows_progress:setPosition(33, 18)
...
function SpineRenderer:createElfinList(  )
    ...
    self.elfin_node:setPosition(-5+20, 3)
    ...
    for i=1,4 do
    	...
    	-- elfin_object.lock_txt:setString(TI18N("语言_c_4177"))
    	...
    	if self.rev_value then
    		...
    		elfin_object.lock_txt:setScaleX(self.rev_value*0.5)
    	end
...
function SpineRenderer:showElfinSkillIcon( data, skills )
	if not self.elfin_node or tolua.isnull(self.elfin_node) then
		...
		for i=1,4 do
			...
			if elfin_data then
				...
				if item_bid == 0 then -- 解锁了，但是未布置精灵
					...
					local elfin_cfg 
					elfin_cfg = Config.SpriteData.data_elfin_data(item_bid)
					if elfin_cfg and elfin_cfg.skill then
						...
						if skill_cfg then
							...
							if not elfin_object.clipNode then
								...
								elfin_object.elfin_icon:setScale(0.28)
	...
	for k,elfin_object in pairs(self.elfin_list) do
		if elfin_object.skill_bid == skill_bid then
			...
			local act_2 = cc.CallFunc:create(function (  )
				if not tolua.isnull(elfin_object.effect_icon) then
...
-- setting.star  英雄星级
-- setting.star_step  英雄星阶330404
-- setting.object_bid  英雄bid
function SpineRenderer:setupUI(group, lev, camp_type, setting)
	...
	local star = setting.star or 0
	local star_step = setting.star_step or 0
	local object_bid = setting.object_bid or 0
	self.height = height
	self.model_x_fix = self.model_x_fix
	self.model_y_fix = self.model_y_fix
	
	if self.shadow == nil and self.spine_type ~= BattleObjectType.Hallows and self.spine_type ~= BattleObjectType.Elfin and self.spine_type ~= BattleObjectType.Jinqi then
	...
	if not BattleController:getInstance():getIsNoramalBattle() or group == BattleGroupTypeConf.TYPE_GROUP_ENEMY then
		if self.ui_hp == nil and self.spine_type ~= BattleObjectType.Hallows and self.spine_type ~= BattleObjectType.Elfin and self.spine_type ~= BattleObjectType.Jinqi then
			self.is_init = true

			self:updateNormalHpUI(self.hp_root_img,59,6,group)	
			...
			if not BattleController:getInstance():getIsNoramalBattle() then
				...
				if star >= 13 and star_step > 0 then
					res = PathTool.getResFrame('battle', 'battle_lev_bg2')
				end
				if star >= 14 then
					res = PathTool.getResFrame('battle', 'battle_lev_bg3')
				end
				if star >= 15 then
					res = PathTool.getResFrame('battle', 'battle_lev_bg4')
				end
				local career_bg = createSprite(res,-20,10, self.hp_root_img, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST)
				career_bg:setCascadeOpacityEnabled(true)
				local res = PathTool.getHeroCampTypeIconByBattle(camp_type)
				if MAKELIFEBETTER_NEW then
					res = PathTool.getHeroCampTypeIcon(camp_type)
				end
				local score = 0
				if group == 1 then
					score = HeroController:getInstance():getModel():heroBidScore(object_bid)
				else
					local cfg = Config.PartnerData.data_partner_base[object_bid]
					if cfg then
						score = cfg.score
					end
				end
				local type_frame = createSprite(res,5,12, career_bg, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
				if score == 9 then
					local sp = createSprite("",type_frame:getContentSize().width*0.5,type_frame:getContentSize().height*0.5, type_frame, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
					if MAKELIFEBETTER_NEW then
						loadSpriteTexture(sp, PathTool.getResFrame("hero/special_hero","special_hero_006"), LOADTEXT_TYPE_PLIST)
					else
						loadSpriteTexture(sp, PathTool.getResFrame("battle","battle_special_hero_006"), LOADTEXT_TYPE_PLIST)
					end
				elseif score == 10 then
					local sp = createSprite("",type_frame:getContentSize().width*0.5,type_frame:getContentSize().height*0.5, type_frame, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
					if MAKELIFEBETTER_NEW then
						loadSpriteTexture(sp, PathTool.getResFrame("hero/ultra_hero","ultra_hero_4"), LOADTEXT_TYPE_PLIST)
					else
						loadSpriteTexture(sp, PathTool.getResFrame("battle","battle_ultra_hero_4"), LOADTEXT_TYPE_PLIST)
					end
				elseif score == 11 then
					local sp = createSprite("",type_frame:getContentSize().width*0.5,type_frame:getContentSize().height*0.5, type_frame, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
					if MAKELIFEBETTER_NEW then
						loadSpriteTexture(sp, PathTool.getResFrame("hero/mystery_hero","mystery_hero_1"), LOADTEXT_TYPE_PLIST)
					else
						loadSpriteTexture(sp, PathTool.getResFrame("battle","battle_mystery_hero_1"), LOADTEXT_TYPE_PLIST)
					end
				end
				type_frame:setScale(0.45)
				if star >= 13 and star_step > 0 then
					type_frame:setPosition(cc.p(30,17))
				end
				if star >= 14 then
					type_frame:setPosition(cc.p(30,17))
				end
				...
				local lev_label
				if MAKELIFEBETTER_NEW then
					lev_label = createWithSystemFont(lev,DEFAULT_FONT, 14)
					lev_label:setPosition(21, 15)
					if star >= 13 and star_step > 0 then
						lev_label:setString(lev.."("..star_step..")")
						lev_label:setPosition(41, 20)
					end
					if star >= 14 then
						if star_step > 0 then
							lev_label:setString(lev.."("..star_step..")")
					...
					lev_label = CommonNum.new(CommonNum.Const_Battle_Level_Num, self.hp_root_img, nil, -2, cc.p(0, 0))
					lev_label:setScale(0.6)
					lev_label:setPosition(cc.p( -28, 12))
					if star >= 13 and star_step > 0 then
						lev_label:setNum(lev.."("..star_step..")")
					else
						lev_label:setNum(lev)
					end
					if crystal > 0  then
						lev_label:setCascadeColorEnabled(true)
						lev_label:setColor(cc.c4b(0x4B,0xFF,0xE8))
					end
				end
			end
		end
	end
	if not self.star_bg and self.spine_type ~= BattleObjectType.Hallows and self.spine_type ~= BattleObjectType.Elfin and self.spine_type ~= BattleObjectType.Jinqi then
		local score = 0
		if group == 1 then
			score = HeroController:getInstance():getModel():heroBidScore(object_bid)
		else
		...
		if score == 10 then
			self.star_bg = createSprite(PathTool.getResFrame("battle","battle_start_1"), -45, -15, self.hp_root, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, -2)
			self.star_txt = createLabel(20, cc.c4b(0xff, 0xff, 0xff, 0xff), cc.c4b(0x33, 0x04, 0x04, 0xff), 14, 14, "", self.star_bg, 2, cc.p(0.5, 0.5))
			self.star_txt:setString(star)
			if star >= 16 then
				if self.star_bg then
					local res = PathTool.getPlistImgForDownLoad("hero","hero_star_ying_icon_4", false)
					loadSpriteTexture(self.star_bg, res, LOADTEXT_TYPE)
					self.star_txt:setString("")
				end
			end
		end
	end
	...
	if status == TRUE then
		if self.resonate_icon == nil then
			if MAKELIFEBETTER_NEW then
				...
				self.resonate_icon = createSprite(PathTool.getResFrame("battle", "battle_common_2037"), 7, 11, self.hp_root_img, cc.p(0.5,0.5), LOADTEXT_TYPE_PLIST)
			end
		end
	else
...
function SpineRenderer:updateNormalHpUI(parent,width,height,group)
	if parent then
		...
		self.ui_hp3 = ccui.LoadingBar:create()
		self.ui_hp3:setCascadeOpacityEnabled(true)
		self.ui_hp3:setScale9Enabled(true)
		self.ui_hp3:setScaleX(-scale)
		self.ui_hp3:setAnchorPoint(cc.p(0, 0.5))
		self.ui_hp3:setPosition(30,1)
		self.ui_hp3:setContentSize(cc.size(width,height))
		self.ui_hp3:setCapInsets(cc.rect(2, 3, 1, 1))
		self.ui_hp3:loadTexture(PathTool.getResFrame("battle", "battle_hp_reduce"), LOADTEXT_TYPE_PLIST)
		parent:addChild(self.ui_hp3,1)
		setChildUnEnabled(true,self.ui_hp3)
		self.ui_hp3:setVisible(false)

		self.ui_hp4 = ccui.LoadingBar:create()
		self.ui_hp4:setCascadeOpacityEnabled(true)
		self.ui_hp4:setScale9Enabled(true)
		self.ui_hp4:setScaleX(scale)
		self.ui_hp4:setAnchorPoint(cc.p(0, 0.5))
		self.ui_hp4:setPosition(-30,-6)
		self.ui_hp4:setContentSize(cc.size(width,4))
		self.ui_hp4:setCapInsets(cc.rect(2, 3, 1, 1))
		self.ui_hp4:loadTexture(PathTool.getResFrame("battle", "battle_shield"), LOADTEXT_TYPE_PLIST)
		parent:addChild(self.ui_hp4,1)
		setChildUnEnabled(true,self.ui_hp4)
		self.ui_hp4:setPercent(0)
		self.ui_hp4:setVisible(false)
		...
		self.ui_hp3:setName("ui_hp3")
		self.ui_hp4:setName("ui_hp4")
	end
end
...
function SpineRenderer:createBuffItem(partner)
	...
	if #self.buff_pool_item > 0 then
		...
		local label
		if MAKELIFEBETTER_NEW then
			label = createLabel(14, Config.ColorData.data_color4[1], nil, 17, -2, 0, icon)
			label:setChangeScaleOffWidth(-1000)
			label:setAnchorPoint(1, 0)
			label:enableOutline(Config.ColorData.data_color4[2], 2)
		else
			label = CommonNum.new(CommonNum.Const_Battle_Buff_Num, icon, nil, -2, cc.p(1, 0))
			label:setScale(0.7)
			label:setPosition(cc.p( 20, 8))
		end
		object = {}
		object.icon = icon
		object.label = label
	...
	for i = 1, length do
		...
		if data then
			...
			if buff_object == nil then
				buff_object = self:createBuffItem(self.hp_root_img) -- 这里有问题了.
			...
			if MAKELIFEBETTER_NEW then
				...
				if data.num and data.num > 1 then
					buff_object.label:setNum(data.num)			-- 显示相同buff的个数
				else
					buff_object.label:setNum("")
				end
				buff_object.icon:setPosition(-20 + ((i - 1)%3) * 22, -19-22 * math.floor((i-1)/3))
				local buff_icon_id = data.res_id
	
				local buffFrame = PathTool.getResFrame('battle', 'battle_buff_icon/'..buff_icon_id)
	
				if buff_object.buff_icon_id ~= buff_icon_id then
					buff_object.buff_icon_id = buff_icon_id 
					loadSpriteTexture(buff_object.icon, buffFrame, LOADTEXT_TYPE_PLIST) 	-- 加载特效资源
				end
			end
		end
	end
...
function SpineRenderer:removeBuffIcon(buff_id)
	...
	if buff_data.num <= 0 then
		...
		if MAKELIFEBETTER_NEW then
			...
			if buff_data.num and buff_data.num > 1 then
				buff_object.label:setNum(buff_data.num)			-- 显示相同buff的个数
			else
				buff_object.label:setNum("")
			end
		end
...
--设置护盾血量
function SpineRenderer:setHpShieldPercent(percent, is_init)
	if not BattleController:getInstance():getModel():getBattleShieldHide() then return end
	if tolua.isnull(self.ui_hp4) then return end
	if percent ~= 0 then
		if percent < 5 then
			percent = 5
		...
		self.ui_hp4:setVisible(true)
	else
		self.ui_hp4:setVisible(false)
	end
	
	if self.ui_hp4:getPercent() <= percent then
		-- 当前动作目标血量值与percent一致，则忽略
		if self.shield_percent and self.shield_percent == percent then return end
		local act_time = BattleController:getInstance():getActTime("hp_effect_time")
		if (self.ui_hp4:getPercent()-percent) >= 60 then -- 进度变化过大，则时间写死0.3秒
			act_time = 0.3
		end
		self.shield_percent = percent
		self.ui_hp4:stopAllActions()
		-- 创建缓慢增长的动作
		local function updateLoadingBar()
			local currentPercent = self.ui_hp4:getPercent()
			if currentPercent < self.shield_percent then
				currentPercent = currentPercent + (100 / (act_time * 60)) -- 每帧增加的百分比
				if currentPercent > self.shield_percent then
					currentPercent = self.shield_percent
				end
				if self.ui_hp4 then
					self.ui_hp4:setPercent(currentPercent)
				end
				return true -- 继续执行
			else
				self.shield_percent = nil
				self.ui_hp4:stopAllActions()
				return false -- 停止执行
			end
		end
		...
		local action = cc.RepeatForever:create(cc.Sequence:create(
			cc.CallFunc:create(updateLoadingBar), -- 调用更新函数
			cc.DelayTime:create(1 / 60) -- 每帧调用一次（假设60帧）
		))

		-- 开始执行动作
		self.ui_hp4:runAction(action)
	else
		self.ui_hp4:stopAllActions()
		self.ui_hp4:setPercent(percent)
	end
end
--设置置灰血量
function SpineRenderer:setHpGreyPercent(percent, is_init)
	if tolua.isnull(self.ui_hp3) then return end
	if percent ~= 0 then
		if percent < 5 then
			percent = 5
		end
		self.ui_hp3:setVisible(true)
	else
		self.ui_hp3:setVisible(false)
	end
	self.ui_hp3:setPercent(percent)
end
--设置血量
...
function SpineRenderer:showHpRoot(bool)
	if bool then
		...
		if not tolua.isnull(self.encircle_effect) then
			self.encircle_effect:setVisible(true)
		end
		...
		for k, v in pairs(self.effect_list or {}) do
			if v then
				for i,effect in pairs(v) do
					if effect.object and effect.object.getName then
						local strList = string.split(effect.object:getName() or "", "_")
						if strList[1] == "BUFF" then
							effect.object:setVisible(true)
						end
					end
				end
			end
		...
		if not tolua.isnull(self.encircle_effect) then
			self.encircle_effect:setVisible(false)
		end
		if not tolua.isnull(self.shadow) then
...
function SpineRenderer:realrelease(bool)
	if bool then
		if not tolua.isnull(self.spine_root) then
			...
			self.spine_root:runAction(cc.Sequence:create(cc.FadeOut:create(0.5), cc.CallFunc:create(function()
				if self.spine then
					self.spine:setVisible(false)
					self.spine:clearTracks()
					self.spine:removeFromParent()
...
function SpineRenderer:setBlack2(enable)
	...
	if enable then
		...
		self.root:runAction(cc.Sequence:create(cc.FadeOut:create(0.2), cc.CallFunc:create(function()
			self.root:setVisible(false)
		end)))
...
--用于特殊召唤出场播放特效
function SpineRenderer:showSpecialEffect(enable)
	if not tolua.isnull(self.unit_effect) then
		self.unit_effect:setVisible(false)
		self.unit_effect:removeFromParent()

		self.unit_effect = nil
	end
	if enable == true then
		local spine_tmp = createSpineByName(Config.EffectData.data_effect_info[1800])
		self.unit_effect = spine_tmp
		self.unit_effect:setAnimation(0, PlayerAction.action, false)
		if self.height then
			spine_tmp:setPosition(0, 0)
		end
		spine_tmp:setVisible(enable)
		...
		local function animationCompleteFunc(event)
			if not tolua.isnull(self.unit_effect) then
				...
				self.unit_effect:runAction(sequence)
			end
		end
		spine_tmp:registerSpineEventHandler(animationCompleteFunc, sp.EventType.ANIMATION_COMPLETE)
	end
end

--用于出场播放特效(替代跑步进场)
function SpineRenderer:showBirthEffect(enable)
	if not tolua.isnull(self.role_birth_effect) then
		self.role_birth_effect:setVisible(false)
		self.role_birth_effect:removeFromParent()

		self.role_birth_effect = nil
	end
	if enable == true then
		local spine_tmp = createSpineByName("E532299")
		self.role_birth_effect = spine_tmp
		self.role_birth_effect:setAnimation(0, PlayerAction.action_1, false)
		local normalFight_voice = Config.PartnerData.data_partner_const_fun("common_flash_voice") 
		if normalFight_voice and normalFight_voice.val and normalFight_voice.val ~= "" then
			AudioManager:getInstance():playEffect(AudioManager.AUDIO_TYPE.BATTLE, normalFight_voice.val, false)
		end
		if self.height then
			spine_tmp:setPosition(0, 0)
		end
		spine_tmp:setVisible(enable)
		self.root:addChild(spine_tmp, 100)
		local function animationCompleteFunc(event)
			if not tolua.isnull(self.role_birth_effect) then
				local sequence = cc.Sequence:create(cc.FadeOut:create(0.1), cc.CallFunc:create(function()
					if not tolua.isnull(self.role_birth_effect) then
				...
				self.role_birth_effect:runAction(sequence)
			end
		end
		spine_tmp:registerSpineEventHandler(animationCompleteFunc, sp.EventType.ANIMATION_COMPLETE)
	end
...
function SpineRenderer:showFuryBar(bl,fury,fury_full)
	self.bl_show_fury_bar = bl
	if self.ui_fury_bg then self.ui_fury_bg:setVisible(bl) end
	-- if self.ui_fury then self.ui_fury:setVisible(bl) end
	if self.ui_fury2 then self.ui_fury2:setVisible(bl) end
end
function SpineRenderer:creatFuryBar(parent,width,height,group)
	if parent then
		local scale = 1
		if self.ui_hp_width then
			scale = width / self.ui_hp_width
		end
		local move_y = self.fury_move_y
		local bar_x = -30
		local bar_y = move_y
		local frame_res = PathTool.getResFrame("battle", "battle_anger_3_1")
		self.ui_fury_bg = createScale9Sprite(frame_res, 0, 1, LOADTEXT_TYPE_PLIST, parent)
		self.ui_fury_bg:setCapInsets(cc.rect(3, 4, 1, 1))
		self.ui_fury_bg:setContentSize(cc.size(61, 6))
		self.ui_fury_bg:setPosition(cc.p(0, move_y))

		
		self.ui_fury2 = cc.ProgressTimer:create(createSprite(PathTool.getResFrame("battle", "battle_anger_1_1"), 0, 0, nil, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST))
		self.ui_fury2:setPosition(bar_x,bar_y)
		-- self.ui_fury2:setOpacity(255)
		self.ui_fury2:setPercentage(0)
		self.ui_fury2:setScaleX(scale)
		self.ui_fury2:setType(cc.PROGRESS_TIMER_TYPE_BAR)
		self.ui_fury2:setBarChangeRate(cc.p(1, 0))
		self.ui_fury2:setAnchorPoint(cc.p(0,0.5))
		self.ui_fury2:setMidpoint(cc.p(0, 0))
		parent:addChild(self.ui_fury2,2)
	end
end
---@隐藏血条 
function SpineRenderer:setHpRootNodeVisible( value )
	value = value or false
	if self.hp_root_img then 
		self.hp_root_img:setVisible(value)
	end 
end 
--==============================--
--desc:播放动作的统一入口
