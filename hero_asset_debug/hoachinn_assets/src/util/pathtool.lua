...
function PathTool.checkRes(path)
	...
	-- print("*****Load Res Image", path)
	if isExist then
		if GmCmd and GmCmd:addLoadList(path) then end
		return path
	else
...
-- 获取觉醒技能的默认底框
function PathTool.getAwakenSkillBg()
	return PathTool.getResFrame("awaken_skill", "common_1005")
end
...
PathTool.AttrIcon = {
	...
	["lev_max"]     = "common_90021_9", --等级上限

	["cntu_dam"]     = "common_90039", --持续伤害增加
	["cntu_res"]     = "common_90021_3", --持续伤害减免
	["def_armor"]     = "common_90021_3", --抗破甲
	["def_hp_hurt"]     = "common_90021_3", --抗生命攻击
	["def_atk_hurt"]     = "common_90021_3", --抗攻击攻击
}
--获取属性图标
...
function PathTool.getQualityBg(quality)
	...
	elseif quality == 7 then
		res_id = "common_1_538"
		return PathTool.getResFrame("common", res_id,false,"common_1")
	elseif quality == 8 then
		res_id = "common_1_603"
		return PathTool.getResFrame("common", res_id,false,"common_1")
	else
		quality = 1005 + quality
		res_id = "common_" .. quality
		return PathTool.getResFrame("common", res_id)
	end
end

-- 获取品质框背景(碎片框)
function PathTool.getChipQualityBg(quality)
	local quality = quality or 1
	if quality > 5 then
		quality = 5
	end
	local res_base = "common_90055_0_" .. quality
	local res_frame = "common_90055_1_" .. quality
	return PathTool.getResFrame("common", res_base), PathTool.getResFrame("common", res_frame)
end
...
function PathTool.getBarQualityBg(quality)
	...
	elseif quality == BackPackConst.quality.green then
		return PathTool.getResFrame("common", "common_1_201",false,"common_1")
...
function PathTool.getWechatSubRes()
	if WECHAT_SUBSCRIPTION_IMG == nil then
		WECHAT_SUBSCRIPTION_IMG = "wechat_subscription_game"
	end
...
function PathTool.getPartnerTypeIcon(_type)
	if _type == (HeroConst and HeroConst.CareerType and HeroConst.CareerType.eOther or 999) then
		return PathTool.getResFrame("common", "common_90104")
	end
	_type = _type or 1
	if _type > 20 then
		_type = _type - 20
...
function PathTool.getHeroCampTypeIcon(_type)
	...
	elseif _type == HeroConst.CampType.eAlien then
		return PathTool.getResFrame("common", "common_90104")
...
---战斗 为了 减少 drawcall ，阵营图标放在了 battle.plist 里面
function PathTool.getHeroCampTypeIconByBattle(_type)
	_type = _type or 1
	if _type == HeroConst.CampType.eWater then --水
		return PathTool.getResFrame("battle", "battle_camp/common_90067")
	elseif _type == HeroConst.CampType.eFire then
		return PathTool.getResFrame("battle", "battle_camp/common_90068")
	elseif _type == HeroConst.CampType.eWind then
		return PathTool.getResFrame("battle", "battle_camp/common_90069")
	elseif _type == HeroConst.CampType.eLight then
		return PathTool.getResFrame("battle", "battle_camp/common_90070")
	elseif _type == HeroConst.CampType.eLingtDark then
		return PathTool.getResFrame("battle", "battle_camp/common_90079")
	elseif _type == HeroConst.CampType.eAlien then
		return PathTool.getResFrame("battle", "battle_camp/common_90104")
	elseif _type == HeroConst.CampType.eDark then
		return PathTool.getResFrame("battle", "battle_camp/common_90071")
	end
end
...
-- 获取联盟徽章图标
function PathTool.getGuildBadgeIcon(str)
	return PathTool.checkRes(string_format("resource/badgeicon/%s.png", str))
end
...
-- 获取大招技能播报头像 名字
function PathTool.getBigSkillImage(head, name)
	local path_head = string_format("resource/heroskill/herohalfdraw/%s.png", head)
	local path_name = string_format("resource/heroskill/skillname/%s.png", name)
	local path_head = PathTool.checkRes(path_head)
	local path_name = PathTool.checkRes(path_name)
	if path_head == PathTool.DEFAULT_IMAGE then
		print(path_head .. "@    不存在")
		path_head = "resource/heroskill/herohalfdraw/0.png"
	end
	if path_name == PathTool.DEFAULT_IMAGE then
		print(path_name .. "@    不存在")
		path_name = "resource/heroskill/skillname/0.png"
	end
	return path_head, path_name
end

-- 获取忍器icon
function PathTool.getSoulHorcruxIcon(res)
	if res == nil or res == 0 then
		res = 113001
	end
	return PathTool.checkRes(string_format("resource/soul/%s.png", res))
end
...
function PathTool.getTargetRes(packageName, resName, is_plist, is_jpg)
	...
	if is_plist then
		...
		if GmCmd and GmCmd:addLoadList(texture) then end
		return plist, texture
	else
		if is_jpg then
			pathurl = PathTool.checkRes(string_format("resource/%s/%s.jpg", packageName, resName))
		else
			pathurl = PathTool.checkRes(string_format("resource/%s/%s.png", packageName, resName))
		end
	end
	if GmCmd and GmCmd:addLoadList(pathurl) then end
	return pathurl
end
...
function PathTool.getPlistImgForDownLoad(packageName, resName, is_jpg)
	local pathurl = ""
	if is_jpg == true then
		pathurl = string_format("resource/%s/%s.jpg", packageName, resName)
	else
		pathurl = string_format("resource/%s/%s.png", packageName, resName)
	end
	if GmCmd and GmCmd:addLoadList(pathurl) then end
	return pathurl
end
...
function PathTool.getTargetCSB(resName)
	-- if DEBUG > 1 then
	if SHOW_GM then
		local csbN = resName..".csb"
		if (not string.find(csbN,"_item.csb")) and (not string.find(csbN,"common/")) then
			print("csb-------------",csbN)
		end
		end
	-- end
...
--勇者之路词条icon
function PathTool.getBraveRoadEntryIcon(tab_index)
	local res_list = {"limit_training_30","limit_training_41","limit_training_42","limit_training_43"}
	return string_format("%s", res_list[tab_index])
end

--魔导器主页icon底图 
function PathTool.getTreasureIconBg(quality,is_left)
	local img_name = ""
	if quality == 1 then
		if is_left == 1 then
			img_name = "ninja_treasure_20"
		else
			img_name = "ninja_treasure_21"
		end
	elseif quality == 2 then
		if is_left == 1 then
			img_name = "ninja_treasure_22"
		else
			img_name = "ninja_treasure_23"
		end
	elseif quality == 3 then
		if is_left == 1 then
			img_name = "ninja_treasure_24"
		else
			img_name = "ninja_treasure_25"
		end
	elseif quality == 4 then
		if is_left == 1 then
			img_name = "ninja_treasure_26"
		else
			img_name = "ninja_treasure_27"
		end
	elseif quality == 5 then
		if is_left == 1 then
			img_name = "ninja_treasure_28"
		else
			img_name = "ninja_treasure_29"
		end
	elseif quality == 6 then
		if is_left == 1 then
			img_name = "ninja_treasure_70"
		else
			img_name = "ninja_treasure_71"
		end
	end
	return PathTool.getResFrame("ninja_treasure",img_name,false,"ninja_treasure")
end
--魔导器抽奖相关资源
function PathTool.getTreasureLottery(str)
	return	PathTool.getResFrame("ninja_treasure/treasure_draw",str,false,"treasure_draw")
end
