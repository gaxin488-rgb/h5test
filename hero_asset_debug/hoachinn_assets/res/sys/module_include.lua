GameModule = {
	"base.baseclass"							--项目中的基类
	, "base.common_ui"							--面板的最基础父类
	, "base.baseview"							--项目中所有窗体的基类，必须要继承与他
	, "base.basecontroller"						--协议处理类的基类

	, "file_update"								--热更判断和逻辑类，闪屏，视频广告等逻辑，url_config配置
	, "call_function"							--设备api,call类方法，操作相册、讯飞语音相关功能，语音，录音、分享、实名验证等
	, "sdk_function"							--平台sdk对接，调用方法类
	, "cli_log"									--日志上报逻辑

-------------全局工具----------
	, "util.pathtool"							--地址、枚举定义与方法类和检测资源是否存在,不存在就判断是否需要从cdn上下载等逻辑
	, "util.util"								--公共工具类
	, "util.gamemath"							--游戏内公式
	, "util.heap"								--二叉堆实现
	, "util.list"								--list类
	, "util.astar"								--A星算法
	, "util.debug"								--调试工具，打印、调试、打印table
	, "util.string_util"						--字符串处理工具类
	, "util.array"								--模拟实现了Array类，游戏数组用它
	, "util.time_tool"							--时间工具类
	, "util.tile_util"							--网格工具类
	, "util.map_util"							--场景工具方法
	, "util.sorttool"							--排序工具类
	, "util.cc_util"							--工具类默认颜色、获取资源数量等
	, "util.fix_array"							--固定长度数组
	, "util.filter"								--滤镜类：饱和度、对比度等
	, "util.cocos_tool"   
	, "util.common_const"                      -- 公共常量或枚举

-------------全局常量-----------
	, "sys.global_const_define"						--事件编号定义,部分游戏内枚举
	, "sys.global_time_ticket"						--用于时间定时的作用，公共定时类模块
	, "sys.global_message_mgr"						--使用于全局的消息的显示：弹幕、跑马灯、泡泡提示
	, "sys.global_keybord_event"					--添加键盘监听回调等方法
	, "sys.word_censor"								--文件功能：敏感词排查和方法
	, "sys.net.smartsocket"							--SmartSocket网络回调处理
	, "sys.net.proto_mgr"							--协议管理,协议
	, "sys.net.gamenet"								--游戏网络相关socket
	, "sys.render_mgr"								--场景渲染管理
	, "sys.jump_controller"							--这里统一处理跳转，跳转界面或功能
	, "sys.sys_controller"							--系统控制，协议请求方法，登陆和断线生连协议请求
	, "sys.sys_env"									--环境变量，环境开关、数据存取读取
	, "sys.role_env"								--环境变量


-------------通用接口-----------
	, "common.common_function"						--公共方法,创建文本、按钮、图片、效果等
	, "common.save_local_data"						--文件功能：用来保存本地数据的一个工具类
	, "common.audio_manager"						--音乐音效控制器
	, "common.view_manager"							--当前项目的各个层级视图
	, "common.global_event"							--自定义事件派发机制
	, "common.common_define"						--公共定义
	, "common.common_alert"							--用于提示的功能
	...
	, "common.debug_tool"							--debug工具
	, "common.debug_value"							--通用协议调试界面
	, "common.error_msg"							--文件功能：专门显示出错信息的界面
	, "common.gm_cmd"								--GM功能界面
	, "common.spine_renderer"						--精灵渲染类
	, "common.tab_select_bar"                       -- 标签页的控件
	, "common.tab_select_btn"                       -- 有选中状态的按钮
	, "common.radio_button"                         -- 单选按钮
	, "common.radio_group"                          -- 单选按钮组
	, "common.player_head"							--通用的头像处理,这里内部有做遮罩处理的
	...
	, "common.resources_cache"						--lua层的资源缓存管理
	, "common.resources_load"						--资源加载控制器
	, "common.resources_load_mgr"					--新的边玩边下控制器，包含了下载模型和下载uI资源
	, "common.common_scrollview"					--自动意义的scrollview
	, "common.common_scrollview_single"				--自动意义的scrollview 单scrollview..支持左右 或者 上下滚动 
	, "common.adjust_bezier_panel"					--调试贝塞尔曲线界面
	, "common.gm_adjust_config_panel"				--gm 修改配置界面

-------------一些纯数据结构类的-----------
	, "game.role.base_role"							--角色数据结构和方法
	, "game.role.role_event"						--角色事件
	, "game.role.looks_vo"							--角色资源数据,主要用于角色的外观形象等
	, "game.role.role_vo"							--主角色的数据vo部分
	, "game.role.role_calculate"					--角色相关的计算方面的问题
	, "game.role.role_const"						--角色相关部分常量
	
--------------功能类------------
	, "game.scene.main_scene_controller"			--新的主中心城控制器
	...
	-- 羁绊宝石
	, "game.gemstone.gemstone_const"
	, "game.gemstone.gemstone_controller"
	, "game.gemstone.gemstone_event"
	, "game.gemstone.gemstone_model"
	, "game.gemstone.gemstone_vo"
	
	, "game.roller.roller_model"
	, "game.roller.roller_controller"
	, "game.roller.roller_const"
	, "game.roller.roller_event"
}
...
NewModelFile = {
	...
	, ["TaskWeekPanel"] = 'game.task.view.task_week_panel'
	...
	-- 选语言界面
	, ["ChooseLangWindow"] = 'game.login.view.choose_lang_window'
	--客服中心
	...
	, ["EnterHillView"] = 'game.story.view.enter_hill_view'
	, ["StoryNewTalk"] = 'game.story.view.story_new_talk'

	, ["MainUiView"] = 'game.mainui.view.main_ui_view'
	, ["TimeView"] = 'game.mainui.view.time_view'
	...
	, ["StarTowerDes"] = 'game.startower.view.star_tower_des'
	...
	, ['HeroHonorWindow'] = 'game.hero.view.hero_honor_window'
	...
	, ['HeroTargetEquipmentSynthesisWindow'] = 'game.hero.view.equip.equip_target_synthesis_window'  --英雄穿戴装备合成
	...
	, ['HeroStarupCostHelpPanel'] = 'game.hero.view.hero_starup_cost_help_panel'
	, ['HeroStarupCostItem'] = 'game.hero.view.hero_starup_cost_help_panel'
	--start 星阶
	, ['HeroMainTabUpgradeStarHigherPanel'] = 'game.hero.view.hero_main_tab_upgrade_star_higher_panel' --升星十一级以上使用
	, ['HeroMainTabUpgradeStarHigherAttrtips'] = 'game.hero.view.hero_main_tab_upgrade_star_higher_attr_tips' --升星十一级以上使用
	, ['HeroUpgradeStarHigherPanel'] = 'game.hero.view.hero_upgrade_star_higher_panel'
	--end
	...
	, ['HeroTipsWindow'] = 'game.hero.view.hero_tips_window'
	, ['HeroFailTipsWindow'] = 'game.hero.view.hero_fail_tips_window'
	, ['HeroFailTipsItem'] = 'game.hero.view.hero_fail_tips_item'
	...
	, ['HeroSacrificeItem'] = 'game.hero.view.hero_sacrifice_item'
	, ['HeroSoulHomologyPanel'] = 'game.hero.view.hero_soul_homology_panel'
	, ['HeroSoulHomologyPreviewPanel'] = 'game.hero.view.hero_soul_homology_preview_panel'
	, ['HeroMainTabSoulHomologyPanel'] = 'game.hero.view.hero_main_tab_soul_homology_panel'
	, ['HeroSoulHomologyCompleteExhibitionPanel'] = 'game.hero.view.hero_soul_homology_complete_exhibition_panel'
	--技能 --by lwc
	...
	, ['HeroGemLearnPanel'] = 'game.hero.view.hero_gem_learn_panel'
	, ['HeroGemLevelUpPanel'] = 'game.hero.view.hero_gem_level_up_panel'
	, ['HeroMainTabGemstonePanel'] = 'game.hero.view.hero_main_tab_gemstone_panel'
	--神装
	...
	, ["DiscordPanel"] = 'game.welfare.view.discord_panel'
	--活动
	...
	, ["ActionWarOrderPanel"] = 'game.action.view.action_war_order_panel'
	, ["ActionWarOrderItem"] = 'game.action.view.action_war_order_item'
	, ["ActionWarOrderAdvancedPanel"] = 'game.action.view.action_war_order_advanced_panel'
	, ["ActionGoodGiftPanel"] = 'game.action.view.action_good_gift_panel'
	...
	, ["LadderChoosePanel"] = 'game.ladder.view.ladder_choose_panel'
	...
	, ["ElfinBookPanel"] = 'game.elfin.view.elfin_book_panel'
	...
	, ["HeroElfinTipsPanel"] = 'game.elfin.view.elfin_hero_tips_panel'
	...
	, ["NewFirstChargeWindow4"] = 'game.newfirstcharge.view.newfirstcharge_window4'
	...
	, ["FindWindow"] = 'game.heaven.view.find_window'
	, ["FindItem"] = 'game.heaven.view.find_item'
	...
	--开服武魂试炼
	, ["KaifuSoulPanel"] = 'game.kaifuActivity.kaifu_soul_panel'
	, ["SoulVideoWindow"] = 'game.kaifuActivity.soul_video_window'
	, ['ElitePanel'] = 'game.elitesummon.view.elite_panel'
	--自选礼包
	, ["ActionOptionalGiftPanel"] = 'game.action.view.action_optional_gift'
	, ["ActionOptionalGiftPanelChose"] = 'game.action.view.action_optional_gift_chose'
	--道具弹窗
	, ["EserciseShowRewardWindow"] = 'game.esecsice.view.esercise_show_reward_window'

	--升星有礼
	, ["ActionRisingStarGiftPanel"] = 'game.action.view.action_RisingStar_gift_panel'

	--大神推荐
	, ["HeroCollocationWindow"] = 'game.hero.view.hero_collocation_window'
	, ["HeroRecommendWindow"] = 'game.hero.view.hero_recommend_window'
	, ["HeroLineupWindow"] = 'game.hero.view.hero_lineup_window'
	, ["HeroLineupItem"] = 'game.hero.view.hero_lineup_item'
	, ["HeroCommentWindow"] = 'game.hero.view.hero_comment_window'
	--服务器
--	,["RoleServerPanel"] = 'game.role.view.role_server_panel'
--	,["RoleServerCell"] = 'game.role.view.role_server_cell'
	
	--羁绊宝石
	, ["GemstoneMainPanel"] = 'game.gemstone.view.gemstone_main_panel'
	, ["GemstoneAwardWindow"] = 'game.gemstone.view.gemstone_award_window'
	, ["GemstoneBagWindow"] = 'game.gemstone.view.gemstone_bag_window'
	, ["GemstoneRemoveWindow"] = 'game.gemstone.view.gemstone_remove_window'
	, ["GemstoneRemoveItem"] = 'game.gemstone.view.gemstone_remove_item'
	, ["GemstoneChoseWindow"] = 'game.gemstone.view.gemstone_chose_window'
	, ["GemstoneEquipPanel"] = 'game.gemstone.view.gemstone_equip_panel'
	, ["GemstoneEquipWindow"] = 'game.gemstone.view.gemstone_equip_window'
	, ["GemstonePreviewWindow"] = 'game.gemstone.view.gemstone_preview_window'
	, ["GemstonePreviewSkillWindow"] = 'game.gemstone.view.gemstone_preview_skill_window'
	, ["GemstoneResonanceWindow"] = 'game.gemstone.view.gemstone_resonance_window'
	, ["GemstoneSkillTipsWindow"] = 'game.gemstone.view.gemstone_skill_tips_window'
	, ["GemstoneSmeltPanel"] = 'game.gemstone.view.gemstone_smelt_panel'
	, ["GemstoneStrengthenWindow"] = 'game.gemstone.view.gemstone_strengthen_window'
	, ["GemstoneTipsWindow"] = 'game.gemstone.view.gemstone_tips_window'
	, ["GemstoneStrengthenItem"] = 'game.gemstone.view.gemstone_strengthen_item'
	, ["GemstonePreviewTipsWindow"] = 'game.gemstone.view.gemstone_preview_tips_window'
	, ["GemstoneRecastPanel"] = 'game.gemstone.view.gemstone_recast_panel'
	, ["GemstoneStrengSelectItemPanel"] = 'game.gemstone.view.gemstone_streng_select_item_panel'
	, ["GemstoneBuffView"] = 'game.gemstone.view.gemstone_buff_view'
	, ["GemstoneResetOfferPanel"] = 'game.gemstone.view.gemstone_reset_offer_panel'
	, ["GemstoneKeyPanel"] = 'game.gemstone.view.gemstone_key_panel'
	-------------------配置表-----------------
	...
	, ["Config_ItemData11"] = "config.item_data11"
	...
	, ["Config_HolidayGetninjaData"] = "config.holiday_getninja_data"
	--音效配置
	...
	-- 章节副本配置
	,["Config_DayGoalsDataArea"] = "config.day_goals_data_area"
	-- 新七日目标
	,["Config_DayGoalsNewDataArea"] = "config.day_goals_new_data_area"
	-- 充值地区配置
	,["Config_ConstantArea"] = "config.constant_area"
	-- 录像馆
   ...
   ,["Config_HolidayValentineBossData"] = "config.holiday_valentine_boss_data"
	...
	--魔法召唤数据
	,["Config_RecruitHolidayMagicData"] = "config.recruit_holiday_magic_data"
	--活动推送页
	...
	--开服武魂试炼
	,["Config_HolidaySoulData"] = "config.holiday_soul_data"
	--大闹天宫
	,["Config_RecruitHolidayHeroData"] = "config.recruit_holiday_hero_data"
	--皮肤之约
	,["Config_RecruitHolidaySkinData"] = "config.recruit_holiday_skin_data"
	--自选礼包
	,["Config_HolidayOptionalGiftData"] = "config.holiday_optional_gift_data"
	--月神之约 --新的七天登录
	,["LoginDaysNewData"] = "config.login_days_new_data"
	--活动战力排行
	,["Config_HolidayRankData"] = "config.holiday_rank_data"
	--潜能释放 sp英雄
	,["Config_PartnerSpecialData"] = "config.partner_special_data"
	,["Config_SkillExtensionData"] = "config.skill_extension_data"
	--羁绊宝石
	,["Config_PartnerGemData"] = "config.partner_gem_data"
}

-- 初始化加载的png
PLIST_LIST = {
	"resource/common/common"
	,"resource/common/common_1"
	,"resource/common/common_2"
