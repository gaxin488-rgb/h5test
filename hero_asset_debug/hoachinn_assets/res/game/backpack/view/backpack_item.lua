...
function BackPackItem:ctor(is_other, click, show_red, scale, effect, is_show_tips, swallow_touch)
	...
	self.receive = self.main_container:getChildByName("receive")
	self.receive:setVisible(false)
	self.MaskImg = self.main_container:getChildByName("MaskImg")
	self.MaskImg:setVisible(false)
	self.Duration = self.MaskImg:getChildByName("Duration")
...
function BackPackItem:setBg(scal)
	self.background:setVisible(scal)
end
function BackPackItem:setNumLabel(scal)
	self.num_label:setVisible(scal)
end
--红点状态
...
--英雄详情是否显示英雄模型
function BackPackItem:setIsHero(scale)
	self.is_hero = scale or false
end
--添加长时间点击的回调
					...
					if self.btn_call_fun then
						...
						if self.is_show_tips and self.data ~= nil then
							...
							elseif BackPackConst.checkIsElfin(type) then -- 精灵
            					TipsManager:getInstance():showGoodsTips(self.data, true, nil, {bag_code = BackPackConst.Bag_Code.PETBACKPACK}) 
							else
								local config
								...
								elseif self.is_tips_source then
									TipsManager:getInstance():showGoodsTips(config, true, self.is_tips_source,nil)
								else
									TipsManager:getInstance():showGoodsTips(config,nil,nil,nil,self.is_hero)
								end
...
function BackPackItem:setSelfEffect(config)
	if not self.is_hide_effect == true then
		if config and config.is_effect and config.is_effect >= 1 then
			...
			self.item_icon:setVisible(true and not self:existChipSprite())
...
function BackPackItem:setItemIcon(head_icon)
	...
	if self.record_head_icon == nil or self.record_head_icon ~= head_icon then
		...
		if self:existChipSprite() then
			loadSpriteTexture(self.chipSprite.icon, head_icon, LOADTEXT_TYPE)
		else
...
function BackPackItem:setBaseData(bid, num, is_spec)
	...
	local head_icon
	if config.is_payval == 1 then
		local res = "txt_cn_"..config.icon
		head_icon = PathTool.getItemRes(res, false)
	else
		head_icon = PathTool.getItemRes(config.icon, false)
	end
	if self.record_head_icon == nil or self.record_head_icon ~= head_icon then
		self.record_head_icon = head_icon
		loadSpriteTexture(self.item_icon, head_icon, LOADTEXT_TYPE)
...
--@return 
--==============================--
function BackPackItem:setData(data,is_hide_effect, is_backpack_item)
	...
	self.is_backpack_item = is_backpack_item or false
	if self.data then
	...
	self.MaskImg:setVisible(false)
	if data and data.config and data.config.id then
		local id = data.config.id
		local cfg = Config.GiftData.data_gift_data[id]
		local item_cfg = Config.ItemData.data_get_data(id)
		if item_cfg and item_cfg.type == 105 then
			local duration = cfg.items[1][2]
			local str1 = (duration / 3600)
			local str = string.format(TI18N("语言_c_4708"),str1)
			self.Duration:setString(str)
	...
	if self.data == nil or id == nil then
        ...
        self.item_icon:setVisible(true)
		local head_icon
		if item_config.is_payval == 1 then
			local res = "txt_cn_"..item_config.icon
			head_icon = PathTool.getItemRes(res, false)
		else
			head_icon = PathTool.getItemRes(item_config.icon, false)
		end
		if self.record_head_icon == nil or self.record_head_icon ~= head_icon then
			self.record_head_icon = head_icon
			loadSpriteTexture(self.item_icon, head_icon, LOADTEXT_TYPE)
	...
	if self.data and self.data.isReceive then
		self.receive:setVisible(true)
	else
		self.receive:setVisible(false)
	end
	self:addVoBindEvent()
	if self.is_backpack_item and data then
		local cfg
		if data.config and data.config.id then
			cfg = Config.ItemData.data_get_data(data.config.id)
		elseif data.bid then
			cfg = Config.ItemData.data_get_data(data.bid)
		end
		if cfg and cfg.type and cfg.type == BackPackConst.item_type.HERO_HUN then
            local starNum = cfg.eqm_jie or 0
            self:createStar(starNum,15)
            self:initCamp(cfg.camp_type)
        else
            self:createStar(0,15)
        end
	else
		self:createStar(0,15)
	end
...
function BackPackItem:setSuitIconPos(posX,posY)
	if self.suit_icon then
		self.suit_icon:setPosition(posX,posY)
	end
end

function BackPackItem:setSuitBtn(status)
	self.suit_icon:setVisible(false)
end
...
function BackPackItem:initCamp(lev)
	...
	if self.camp_icom then
		...
		elseif self.item_config then
			cfg = self.item_config
		end
		...
		if next(cfg) then
			if cfg.effect and cfg.effect[1] and cfg.effect[1].val then
				local id = 0
				if type(cfg.effect[1].val) == "table" then
					id = cfg.effect[1].val[1]
				else
					id = cfg.effect[1].val
				end
				local hero_cfg = Config.PartnerData.data_partner_base[id]
				if hero_cfg and hero_cfg.score >= 9 then
					if not self.sp then
						self.sp = createSprite("",self.camp_icom:getContentSize().width*0.5,self.camp_icom:getContentSize().height*0.5, self.camp_icom, cc.p(0.5, 0.5), LOADTEXT_TYPE_PLIST, 3)
					end
...
function BackPackItem:isChipSprite(config)
	if config.type == BackPackConst.item_type.PARTNER_DEBRIS then
		local res_base,res_frame = PathTool.getChipQualityBg(config.quality)
		if config.effect and config.effect[1] and config.effect[1].val then
			local hero_cfg = Config.PartnerData.data_partner_base[config.effect[1].val]
			if hero_cfg then
				if hero_cfg.score == 9 then
					res_base = PathTool.getResFrame("common", "common_90055_0_5")
					res_frame = PathTool.getResFrame("common", "common_90055_1_5")
				elseif hero_cfg.score == 10 then
					res_base = PathTool.getResFrame("common", "common_90055_0_7")
					res_frame = PathTool.getResFrame("common", "common_90055_1_7")
				elseif hero_cfg.score == 11 then
					res_base = PathTool.getResFrame("common", "common_90055_0_8")
					res_frame = PathTool.getResFrame("common", "common_90055_1_8")
				end
			end
		end
		if not self.chipSprite then
			self.chipSprite = createSprite("", 60, 60, self.main_container, cc.p(0.5,0.5), LOADTEXT_TYPE_PLIST, 0)
			...
			self.chipSprite.icon = createImage(icon_clipNode, self.record_head_icon, 53, 53, cc.p(0.5, 0.5), false)
			self.chipSprite.top_img = createSprite("", 53, 53, self.chipSprite, cc.p(0.5,0.5), LOADTEXT_TYPE_PLIST, 0)
		end
		if self.chipSprite then
			...
			self.chipSprite.icon:loadTexture(self.record_head_icon, LOADTEXT_TYPE)
...
function BackPackItem:existChipSprite()
...
function BackPackItem:createCompLayer()
	if not self.compLayer then
	    ...
	    local bg,comp_bar = createLoadingBar(res, res1, cc.size(107, 19), self.compLayer, cc.p(0,0.5), 0, self.compLayer:getContentSize().height/2, true, true ,0,0)
		...
		self.text_bar:enableOutline(cc.c3b(0x54,0x42,0x34),2)
	end
end
...
function BackPackItem:setGemstoneLev(lv)
	...
	if lv <= 0 then
		...
		self.gemstone_label:setString(string.format(TI18N("语言_c_4712"),lv))
...
function BackPackItem:createStar(num,width_s)
...
function BackPackItem:setEquipLev(lev,scale,font_size,pos)
    ...
    self.lev_label:setString(string.format(TI18N("语言_c_4712"),lev))
...
--战令活动的个锁背景
function BackPackItem:showOrderWarLockBg(bool)
	if not self.order_war_lock_bg then 
		-- local temp_pos = pos or cc.p(7,97)
		-- self.order_war_lock_bg = createSprite(PathTool.getResFrame("crossground","crossground_1002"), temp_pos.x,temp_pos.y, self.root_wnd, cc.p(0.5,0.5))
		self.order_war_lock_bg = createImage(self.root_wnd,PathTool.getResFrame("crossground","crossground_1002"), 0,0, cc.p(0,0),true,9998,true)
		
		self.order_war_lock_bg:setCapInsets(cc.rect(14,14,1,1))
		self.order_war_lock_bg:setContentSize(cc.size(119,119))
		if self.order_war_lock then
			self.order_war_lock:setZOrder(9999)
		end
	end
	self.order_war_lock_bg:setVisible(bool)
end
--战令活动 物品是否领取状态
...
--增加一个名字，默认位置为底部
function BackPackItem:setGoodsName(name,pos,font_size,color,line_color,scale, max_width)
	...
	if not self.goods_name  then 
		self.goods_name = createLabel(font_size,item_color,line_color,self.Width/2,-30,"  ",self.root_wnd,2, cc.p(0.5,1))
		self.goods_name:setScale(1 / scale)
		self.goods_name:setAlignment(cc.TEXT_ALIGNMENT_CENTER)
		self.goods_name.off_y = self.goods_name:getContentSize().height
		self.goods_name:setPositionY(self.goods_name:getPositionY() + self.goods_name.off_y)
	end
	name = name or ""
	self.goods_name:setString(name)
	if pos then 
		pos.y = pos.y + self.goods_name.off_y
		self.goods_name:setPosition(pos)
	end
	if max_width then
		self.goods_name:setDimensions(max_width, 0)
		self.goods_name:setLineSpacing(-6)
	end
end
...
function BackPackItem:showRedPoint(status)
    ...
    if not self.red_point then 
        ...
        self.red_point =createImage(self,res,92,92,cc.p(0,0),true,10,false)
...
function BackPackItem:showGainTag(status,scale)
    ...
    if not self.tag_point then
		...
		self.tag_label = createLabel(18, 1, cc.c4b(0x87,0x18,0x00,0xff), 29, 32, TI18N("语言_c_3400"), self.tag_point, 1, cc.p(0.5, 0))
...
function BackPackItem:showDoubleTag(status,num)
	if not self.double_ponit then
		...
		self.use_ponit_label = createLabel(18, 1, cc.c4b(0x87,0x18,0x00,0xff), 29, 32, TI18N("语言_c_3401"), self.tag_point, 1, cc.p(0.5, 0))
...
-- 穿戴装备能够合成更高级别显示
function BackPackItem:setHeroEquipComposeMark(status, call_back)
    if status then
        if self.equip_comp_mark == nil then
            local res = PathTool.getResFrame("common","common_zbjiantou")
            self.equip_comp_mark = createImage(self, res, 10, self.size.height-20, cc.p(0.5, 0.5), true)
            self.equip_comp_mark:setScale(1.25)
        else
            self.equip_comp_mark:setVisible(true)
        end
	    self.equip_comp_mark:addTouchEventListener(function(sender, event_type) 
	        if event_type == ccui.TouchEventType.ended then
	            playButtonSound2()
	            if call_back then
	            	call_back()
	    ...
	    self.equip_comp_mark:setTouchEnabled(true)
    else
        if self.equip_comp_mark then
            self.equip_comp_mark:setVisible(false)
        end
    end
end
