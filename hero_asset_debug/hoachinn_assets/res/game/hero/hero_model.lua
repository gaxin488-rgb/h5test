...
function HeroModel:config()
    ...
    --符文解锁条件信息
local function getArtifactLimit(key)
        if Config.PartnerData.data_partner_const[key] then
            return Config.PartnerData.data_partner_const[key].val
        end
        local default_limit = {
            artifact_three = {'star', 150},
            artifact_four = {'star', 200},
            artifact_five = {'star', 250},
            artifact_six = {'star', 300},
        }
        return default_limit[key] or {'lev', 1}
    end

    local artifact_one = getArtifactLimit("artifact_one")
    local artifact_two = getArtifactLimit("artifact_two")
    -- 对应新增的 9, 10, 11, 12 号位
    local artifact_three = getArtifactLimit("artifact_three") 
    local artifact_four = getArtifactLimit("artifact_four")
    local artifact_five = getArtifactLimit("artifact_five")
    local artifact_six = getArtifactLimit("artifact_six")

    self.artifact_lock_list = {artifact_one, artifact_two, artifact_three, artifact_four, artifact_five, artifact_six}
    -- [修改结束]
...
function HeroModel:isMaxStarHero(bid, star)
    ...
    local client_star_limit = 255
    if star < client_star_limit then
        return false
    end
