...
function BackPackWindow:__init(sub_type)
    ...
    self.res_list = {
        ...
        { path = PathTool.getPlistImgForDownLoad("bigbg","bigbg_101",true), type = ResourcesType.single },
...
function BackPackWindow:open_callback()
    ...
    if self.background ~= nil then
        self.background:loadTexture(PathTool.getPlistImgForDownLoad("bigbg","bigbg_101",true), LOADTEXT_TYPE)
    ...
    self.quick_sell_btn:getChildByName("label"):setString(TI18N("语言_c_1799"))
    ...
    local tab_array = {
        {title = TI18N("语言_c_969"), index = BackPackConst.item_tab_type.EQUIPS},
        {title = TI18N("语言_c_408"), index = BackPackConst.item_tab_type.PROPS},
        {title = TI18N("语言_c_1706"), index = BackPackConst.item_tab_type.HERO},
        {title = TI18N("语言_c_1716"), index = BackPackConst.item_tab_type.SPECIAL},
        {title = TI18N("语言_ite_6_td_1"), index = BackPackConst.item_tab_type.ELFIN},
        {title = TI18N("语言_exc_02_24"), index = BackPackConst.item_tab_type.HOLYEQUIPMENT},
    }
    ...
    local scroll_view_size = cc.size(670,580)
    local setting = {
    ...
    self.item_scrollview = CommonScrollViewLayout.new(self.container, cc.p(25, 185), ScrollViewDir.vertical, ScrollViewStartPos.top, scroll_view_size, setting)
...
function BackPackWindow:createTitleEffect()
	...
	self.title_effect = createEffectSpine(PathTool.getEffectRes(640), cc.p(-122, -6), cc.p(0, 0), true, PlayerAction.action_1)
...
function BackPackWindow:openRootWnd()
	...
	status = self:checkHolyEqmIsFull()
	if status then
		local msg = TI18N("The Holy Equipment Bag is about to be full, please sell but the equipment is not necessary to avoid errors")
		CommonAlert.show(msg, TI18N("语言_c_63"), applyFun, nil, nil, nil, nil,{is_auto_close = true})
	end
end

function BackPackWindow:checkHolyEqmIsFull()
    local max_size = 350
    local list = model:getAllBackPackArray(BackPackConst.item_tab_type.HOLYEQUIPMENT)
...
-- Gop cac dot 10510 lien tiep thanh mot lan refresh UI, tranh sort + setData lien tuc
-- khi server dang stream mot tui rat lon theo nhieu packet nho.
function BackPackWindow:queuePanelRefresh()
    if self._bag_refresh_waiting then return end
    self._bag_refresh_waiting = true
    if self.root_wnd and not tolua.isnull(self.root_wnd) then
        delayRun(self.root_wnd, 0.12, function()
            self._bag_refresh_waiting = false
            if self.item_scrollview and self.cur_index then
                self:setPanelData(false)
            end
        end)
    else
...
function BackPackWindow:setPanelData()
    ...
    for key, value in pairs(self.item_scrollview:getItemList()) do
        value:createStar(0,15)
        if value and value.data then
            local cfg = Config.ItemData.data_get_data(value.data.config.id)
            if cfg and cfg.type and cfg.type == BackPackConst.item_type.HERO_HUN then
                local starNum = cfg.eqm_jie or 0
                value:createStar(starNum,15)
    ...
    if self.cur_index == BackPackConst.item_tab_type.HERO then
        self.quick_sell_btn:getChildByName("label"):setString(TI18N("语言_c_3215"))
    else
        self.quick_sell_btn:getChildByName("label"):setString(TI18N("语言_c_1799"))
    end
...
function BackPackWindow:selectedItem(cell, force)
    if cell and cell:getData() then 
        ...
        elseif BackPackConst.checkIsElfin(vo.config.type) then -- 精灵
            TipsManager:getInstance():showGoodsTips(vo, true, nil, {bag_code = BackPackConst.Bag_Code.PETBACKPACK}) 
        else
